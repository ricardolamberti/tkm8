...
// $Id: calendar.js,v 1.3 2008/01/09 13:26:54 rlamberti Exp $
...
Calendar.getAbsolutePos = function(el) {
	...
	if (el.getBoundingClientRect) {
        ...
        r.x = box.left + scrollLeft;
        r.y = box.top + scrollTop;
        return r;
    }
	if (el.offsetParent) {
...
Calendar.cellClick = function(el) {
	...
	if (newdate) {
		cal.callHandler();
		// RJL parche a lo de SGALLI
		if (mustClose) {
			// SGALLI : Close calendar on single click
			Calendar.removeClass(el, "hilite");
			cal.callCloseHandler();
		}
...
Calendar._keyEvent = function(ev) {
    // ZEUS: added to allow a key event being disabled
    if (ev.keyCode && ev.keyCode==-1) {
        return false;
    }
