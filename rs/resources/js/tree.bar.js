...
// ***************************************************************** 
function Folder(folderDescription, hreference, actOwner, action) //constructor 
{  
  //constant data 
...
//  this.isLastNode = 0;
  ...
  this.addChildOnLine = addChildOnLine
  this.expandTreeOnLine = expandTreeOnLine
  this.getLeftSide = getLeftSide
  this.removeAll = removeAll
  this.remove = remove	
  this.isEmpty = isEmpty	
  this.lastNode = lastNode	
  this.createDummy = createDummy	
  this.hasToExpand = hasToExpand	
  this.setExpandAllways = setExpandAllways	
} 

function hasToExpand() {
  if (this.nChildren==1 && this.children[0].getDesc()==dummy.getDesc()) 
    return true;
  if (this.expandAllways=='true') {
  	this.removeAll();
  	this.createDummy();
  	this.folderMstr(this.getNavObj());
  	this.setState(true);
    return true;
  }
  return false;
...
function expandTreeOnLine() {
 	if (!this.hasToExpand()) return;
	var ajax = new ZAjaxSubmit("do-TreeExpandNode",false);
	ajax.addUrlParameters("dg_act_owner", this.actOwner);
    ajax.addUrlParameters("dg_action", this.action);
	waitingParent = this;
	ajax.setAjaxCallback(this.onAjaxRecieve,genericErrorMessage);
	ajax.fetch();
}

function registerDummy(zdummy) {
	dummy=zdummy;
}

function createDummy() {
  var fdummy=gFld(dummy.getDesc(), '', '', '');
  fdummy.iconSrc = dummy.iconSrc;
  fdummy.iconSrcClosed = dummy.iconSrcClosed;
  fdummy.initialize(this.getLevel()+1, true, this.getLeftSide()+"1");
  this.addChild(fdummy);
}


function onAjaxRecieve(ajaxResponse) {
//alert(ajaxResponse);
  waitingParent.removeAll();
...
function remove() {
  var element = getElById("folder"+this.id);
...
function addChildOnLine(newFld, hasChildren) 
{
  var first = waitingParent.isEmpty();
  var left = "1";
  if (waitingParent.lastNode()) left = "0";
  waitingParent.addChild(newFld);
  newFld.initialize(waitingParent.getLevel()+1, first, waitingParent.getLeftSide()+left);
  if (hasChildren) newFld.createDummy();
  newFld.folderMstr(waitingParent.getNavObj());  
  waitingParent.setState(true);
}
...
{ 
  ...
  if (browserVersion > 0) 
  // ZEUS: added class for skin
    auxEv = "<a class='tree_folder_node' " +
             getNullTooltipHTML()
            + " onClick='javascript:clickOnNode(\""+this.getID()+"\");window.status=\"\";return true;'>" 
  else 
    auxEv = "<a>" 
...
{ 
  ...
  if (this.hreference && isLinked(this.hreference)) 
  { 
	if (USEFRAMES)
	// PSS: added class for skin
	  docW = docW + "<a class='tree_folder_node' " + 
	  	getTooltipHTML(this.hreference, this.desc) + " href='" + this.hreference + "' TARGET=\"basefrm\" "
	else
	// PSS: added class for skin
	  docW = docW + "<a class='tree_folder_node' " + 
	  	getTooltipHTML(this.hreference, this.desc) + " href='" + this.hreference + "' TARGET=_top "
    ...
    if (browserVersion > 0) {
      docW = docW + "onClick='javascript:clickOnFolder(\""+this.getID()+"\");window.status=\"\";return true;'"
    }

    docW = docW + ">"
  } 
  else 
    docW = docW + "<div class='tree_folder_node' " + getTooltipHTML(this.hreference, this.desc) + ">" 
...
function getTooltipHTML(ref,nodeDesc) {
  return " title='  " +nodeDesc +
  		"' onmouseover=\"return showTooltipText(" + isLinked(ref) + ",'" + nodeDesc + "');\"";
}
function getNullTooltipHTML() {
  return " onmouseover=\"return showTooltipText(false,'');\"";
}

function showTooltipText(selectable,nodeDesc) {
  var tooltip;
  if (selectable) {
    tooltip = nodeDesc;
  } else {
...
// ZEUS: added class for skin and build differently the link url
//  original code is:
//   var fullLink = "href=\""+this.link+"\" target=\""+this.target+"\" onClick=\"clickOnLink('"+this.getID()+"\', '"+this.link+"','"+this.target+"');window.status=\"\";return true;\"";

  var fullLink = " class='tree_leave_node' ";
  fullLink += getTooltipHTML(this.link, this.desc);
  fullLink += " href='"+this.link+"'";
  fullLink += " target=\""+this.target+"\"";
  fullLink += " onClick=\"clickOnLink(\'"+this.getID()+"\', \'javascript:undefined\',\'"+this.target+"\');window.status='';return true;\"";
...
function setExpandAllways(value)
{
  this.expandAllways=value;
}
...
function isLinked(hrefText) {
    var result = true;
    if (typeof hrefText == "undefined") return false;
...
function gFld(description, hreference, actOwner, action) 
{ 
//alert('des='+description);
//alert('dref='+hreference);
//alert('own='+actOwner);
//alert('act='+action);
  folder = new Folder(description, hreference, actOwner, action);
  return folder;
} 
...
function insFld(parentFolder, childFolder, hasChildren) 
{ 
//  alert("padre: "+ parentFolder.getDesc() +" -- hijo: "+ childFolder.getDesc() + " creoDummy="+hasChildren);
  var child = parentFolder.addChild(childFolder) 
  if (hasChildren) child.createDummy();
  return child;
} 
    ...
    for (fldPos=0; fldPos<fldArr.length; fldPos++)
    {
      ...
      if (fldStr != "") {
        ...
        if (nodeObj!=null) //may have been deleted
          ...
          // ZEUS: ignore this alert
          /*else
            alert("Internal id '" + fldArr + "' is not pointing to a folder anymore. Consider using external IDs")*/
      }
    }
  }
}
...
// ZEUS: added to set current selection from server
function setPssSelection(zSelection) {
if (GetCookie('pssTreeSelection')=='ready') return;
SetCookie('pssTreeSelection', zSelection);
}
...
function initializeDocument() 
{
  ...
  // ZEUS: added to set current selection from server when session starts
  /*
  var sCurrSelection = GetCookie('pssTreeSelection');
  if (document.treeDataById != null && sCurrSelection!="ready") {
    var bSelectionSet = false;
  	for (i=0; i < document.treeDataById.length; i++) {
  	  if (document.treeDataById[i]==sCurrSelection) {
  	    SetCookie('highlightedTreeviewLink', i);
  	    bSelectionSet = true;
  	    break;
  	  }
  	}
