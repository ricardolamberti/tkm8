...
<xsl:template match="message">
	<xsl:if test="text/.!=''">
		<script>
			showErrRefresh('<xsl:value-of select="@type"/>','<xsl:value-of select="@title"/>','<xsl:value-of select="text/."/>');
		</script>
	</xsl:if>
	...
	<xsl:choose>
		...
		<xsl:when test="@orientation='V'">
		    <div>
		        ...
		        <div class="panel-left">
		        	<xsl:attribute name="id">panel-left_<xsl:value-of select="@name"/></xsl:attribute>
		        ...
		        <div class="splitter">
		        	<xsl:attribute name="id">splitter_<xsl:value-of select="@name"/></xsl:attribute>
		        </div>
		
		        <div class="panel-right">
		        	<xsl:attribute name="id">panel-right_<xsl:value-of select="@name"/></xsl:attribute>
		...
		<xsl:otherwise>
		    <div class="panel-container-vertical">
		        ...
		        <div class="panel-top">
		        	<xsl:attribute name="id">panel-top_<xsl:value-of select="@name"/></xsl:attribute>
					<xsl:for-each select="panel_a">
						<xsl:call-template name="basic_generate_component_responsive"/>
					</xsl:for-each>
		        </div>
		        ...
		        <div class="splitter-horizontal">
		        	<xsl:attribute name="id">splitter-horizontal_<xsl:value-of select="@name"/></xsl:attribute>
		        	<xsl:attribute name="id">splitter-horizontal</xsl:attribute>
		        </div>
		
		        <div class="panel-bottom">
		        	<xsl:attribute name="id">panel-bottom_<xsl:value-of select="@name"/></xsl:attribute>
					<xsl:for-each select="panel_b">
						<xsl:call-template name="basic_generate_component_responsive"/>
					</xsl:for-each>
		        </div>
...
<xsl:template match="card_responsive">
	...
	<div>
		...
		<input type="hidden">
    	   	<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_zobject</xsl:attribute>
        	<xsl:attribute name="value"><xsl:value-of select="@zobject"/></xsl:attribute>
		</input>	 
		<div>
			<xsl:call-template name="basic_generate_component_responsive"/>
	          ...
	          <div class="card-body">
	               <div class="row gutter-0">
	                   ...
	                   <div>
	                       ...
	                       <div class="infocard"><xsl:value-of select="text/."/></div>
...
<xsl:template name="view_image_responsive">
		<xsl:choose>
			<xsl:when test="@diferido='true'">
				<div>
					<xsl:call-template name="basic_generate_component_responsive"/>
					<div>
						<xsl:attribute name="id"><xsl:value-of select="image/@graph_content"/></xsl:attribute>
						<xsl:attribute name="style"><xsl:call-template name="generate_inline_style_responsive"/></xsl:attribute>
					</div>
				</div>
				<script type="POSTSCRIPT">
		...
		<xsl:choose>
			<xsl:when test="icon">
					<xsl:call-template name="render_icon">
						<xsl:with-param name="style_attr"><xsl:if test="icon/@style_image=''">width:100%;</xsl:if></xsl:with-param>
					</xsl:call-template>
			</xsl:when>
           ...
           <xsl:if test="action">
	           ...
	           <a >
								<xsl:if test="@refreshForm='true'">
										<xsl:for-each select="action">
											<xsl:attribute name="onClick">
												goToRefreshForm('do-PartialRefreshForm', '<xsl:value-of select="../@name"/>', '<xsl:value-of select="../action/@obj_provider"/>','<xsl:value-of select="../action/@object_owner_id"/>', getLastModal(), '<xsl:value-of select="../action/@id_action"/>');
...
<xsl:template match="h1_responsive">
	<H1>
		<xsl:call-template name="basic_generate_component_responsive">
			<xsl:with-param name="with_title" select="'true'" />
		</xsl:call-template>
 	</H1>
</xsl:template>
...
<xsl:template match="win_expand_responsive">
	...
	<xsl:if test="/page/header/savehelp">			
	  	...
	  	<xsl:if test="not(@class_responsive)"><xsl:attribute name="class">col-sm-12</xsl:attribute></xsl:if>
	  	<xsl:attribute name="style">min-height:10000px</xsl:attribute>
		<xsl:for-each select="rows">
			<xsl:for-each select="row">
					<div >
						<xsl:attribute name="id"><xsl:value-of select="@id"/>_move</xsl:attribute>
						<xsl:attribute name="class">zone_draggeable</xsl:attribute>
						<div style="display:inline-block">
							<xsl:attribute name="id"><xsl:value-of select="@id"/>_resize</xsl:attribute>
							<xsl:attribute name="class">zone_resizeable</xsl:attribute>
							<div>
								<xsl:attribute name="id"><xsl:value-of select="@id"/></xsl:attribute>
							  	<xsl:if test="not(@resize) and @class_responsive">
							  		<xsl:attribute name="class"><xsl:value-of select="@class_responsive"/></xsl:attribute>
							  	</xsl:if>
							  	<script type="POSTSCRIPT">
			        				<xsl:for-each select="action">
											<xsl:call-template name="generate_action_invokation"/>
					...
					<input type="hidden">
						...
						<xsl:attribute name="value"><xsl:value-of select="@cv_posy"/></xsl:attribute>
					</input>
		  		    <input type="hidden">
						...
						<xsl:attribute name="value"><xsl:value-of select="@cv_posx"/></xsl:attribute>
					</input>
		  		    <input type="hidden">
						...
						<xsl:attribute name="value"><xsl:value-of select="@cv_width"/></xsl:attribute>
					</input>
		  		    <input type="hidden">
						...
						<xsl:attribute name="value"><xsl:value-of select="@cv_height"/></xsl:attribute>
					</input>
		
					<script>
						 $( function() {
								registerMeta('dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@id"/>_posy',['dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@id"/>_posy']);
								registerMeta('dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@id"/>_posx',['dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@id"/>_posx']);
								registerMeta('dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@id"/>_width',['dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@id"/>_width']);
								registerMeta('dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@id"/>_height',['dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@id"/>_height']);
							    
							    $('#<xsl:value-of select="@id"/>_resize').resize("resize.<xsl:value-of select="@id"/>",function(){
							        $("#dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@id"/>_width").val($("#<xsl:value-of select="@id"/>_move").width());
							        $("#dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@id"/>_height").val($("#<xsl:value-of select="@id"/>_move").height());
							        });
								$('#<xsl:value-of select="@id"/>_move').on('dragstop.<xsl:value-of select="@id"/>', function (event) {
									$(function() {		      
								        $("#dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@id"/>_posy").val($("#<xsl:value-of select="@id"/>_resize").position().top);
								        $("#dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@id"/>_posx").val($("#<xsl:value-of select="@id"/>_resize").position().left);
									});
										
								});
								
								$("#<xsl:value-of select="@id"/>_move").css( {"top":"<xsl:value-of select="@cv_posy"/>px", "left":"<xsl:value-of select="@cv_posx"/>px","width":"<xsl:value-of select="@cv_width"/>px", "height":"<xsl:value-of select="@cv_height"/>px"});
		        				$("#<xsl:value-of select="@id"/>_resize").resizable(); 
		        				$("#<xsl:value-of select="@id"/>_move").draggable({ scroll: true, containment: "parent" });
						});
...
<xsl:template match="win_row_expand_responsive">
	<xsl:choose>
		<xsl:when test="@expanded='true'">
			<div >
				<xsl:attribute name="id"><xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>_move</xsl:attribute>
				<xsl:if test="@resizable='true' and @editable='true'"><xsl:attribute name="class">zone_draggeable</xsl:attribute></xsl:if>
				<div>
					<xsl:attribute name="id"><xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>_resize</xsl:attribute>
					<xsl:if test="@resizable='true'">
						<xsl:attribute name="style">display:inline-block;</xsl:attribute>
						<xsl:if test="@editable='true'"><xsl:attribute name="class">zone_resizeable</xsl:attribute></xsl:if>
						<xsl:attribute name="onresize">
						   javascript:zoomToFit('<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>_resize','<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>')
						</xsl:attribute>
					</xsl:if>
					<div>
						<xsl:attribute name="id"><xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/></xsl:attribute>
						<div>
							<xsl:call-template name="basic_generate_component_responsive"/>
						</div>
				...
				<script type="POSTSCRIPT">
					 $( function() {
							<xsl:if test="@editable='true'">
								$('#<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>_resize').resize("resize.<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>",function(){
							        setJQValue("#dgf_<xsl:value-of select="@form_name"/>_fd-width",$("#<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>_resize").width());
					 		        setJQValue("#dgf_<xsl:value-of select="@form_name"/>_fd-height",$("#<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>_resize").height());
							        setJQValue("#dgf_<xsl:value-of select="@form_name"/>_fd-posy",parseInt($("#<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>_move").css("top"), 10) || 0);
							        setJQValue("#dgf_<xsl:value-of select="@form_name"/>_fd-posx",parseInt($("#<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>_move").css("left"), 10) || 0);
							        });
								$('#<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>_move').on('dragstop.<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>', function (event) {
									$(function() {		      
								        setJQValue("#dgf_<xsl:value-of select="@form_name"/>_fd-posy",parseInt($("#<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>_move").css("top"), 10) || 0);
								        setJQValue("#dgf_<xsl:value-of select="@form_name"/>_fd-posx",parseInt($("#<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>_move").css("left"), 10) || 0);
									});
			       				...
			       				$("#<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>_resize").resizable(); 
			       				$("#<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>_move").draggable({ scroll: true, containment: "parent" });
		       				</xsl:if>
							$("#<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>_move").css( {"top":"<xsl:value-of select="@cv_posy"/>px", "left":"<xsl:value-of select="@cv_posx"/>px"});
							$("#<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>_resize").css( {"width":"<xsl:value-of select="@cv_width"/>px", "height":"<xsl:value-of select="@cv_height"/>px"});
							zoomToFit('<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>_resize','<xsl:value-of select="@name_parent"/>_<xsl:value-of select="@rowid"/>')
					});
				</script>			
			</xsl:if>
		...
		<xsl:otherwise>
			<div>
		      ...
		      <xsl:attribute name="class">flex-col panel<xsl:text><![CDATA[ ]]></xsl:text><xsl:value-of select="@class_header_responsive"/>
		      <xsl:text><![CDATA[ ]]></xsl:text><xsl:if test="not(@visible and @visible='true')">hidden</xsl:if></xsl:attribute>
		      <div class="panel-heading no-margin">
		        ...
		        <h4 class="panel-title">
		          <a data-toggle="collapse">
			      	<xsl:attribute name="data-parent">#<xsl:value-of select="@name_parent"/></xsl:attribute>
			      	<xsl:attribute name="href">#<xsl:value-of select="@name_parent"/>collapse<xsl:value-of select="@rowid"/></xsl:attribute>
		          ...
		          <xsl:if test="@label_right or @label_right!=''">
		         	 <div class="panel-info" style="float:right"><xsl:value-of select="@label_right"/></div>
		          </xsl:if>
				       <xsl:if test="@toolbarpos='in'">
				          <div style="float:right"  class="panel-info" >
				          	<xsl:attribute name="id"><xsl:value-of select="@provider_name"/>_toolbar</xsl:attribute>
				          </div>
							 </xsl:if>
		        </h4> 
		      </div>
		      <div class="panel-collapse collapse">
		      	<xsl:attribute name="id"><xsl:value-of select="@name_parent"/>collapse<xsl:value-of select="@rowid"/></xsl:attribute>
		       ...
		       <xsl:if test="@toolbarpos='in'">
						<script>
					 		if ($('#actionbarinternal_<xsl:value-of select="@provider_name"/>').html()!="") {
						     $('#<xsl:value-of select="@provider_name"/>_toolbar').html($('#actionbarinternal_<xsl:value-of select="@provider_name"/>').html());
							   $('#actionbarinternal_<xsl:value-of select="@provider_name"/>').html(""); 
						    }
						</script>	
					</xsl:if>
		</xsl:otherwise>
...
<xsl:template match="panel_responsive">
   	<div>
		<xsl:if test="@zoomtofit">
   		...
   		<xsl:attribute name="class">
   			<xsl:value-of select="@size_responsive"></xsl:value-of>
			<xsl:if test="@dataparent">
 				  	   <xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>collapse
   			</xsl:if>
   		</xsl:attribute>    
		<xsl:if test="@height">
	        ...
	        <div>
	          	<xsl:attribute name="class">
		           	<xsl:choose>
						<xsl:when test="@visible and (@visible='true')">
							form-group<xsl:if test="constraints/@required='true'"><![CDATA[ ]]>required</xsl:if>
						</xsl:when>
				...
				<div>
				  <xsl:if test="@class_responsive!='well-white' and @class_responsive!='row'">
			   		  <xsl:attribute name="class"><xsl:value-of select="@class_responsive"></xsl:value-of></xsl:attribute>
			      </xsl:if>    
				  <xsl:if test="@label">
...
<xsl:template match="panel_resizable_responsive">
 	<xsl:variable name="fullname">dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@id"/></xsl:variable>
	<div >
		<xsl:attribute name="id"><xsl:value-of select="@id"/>_move</xsl:attribute>
		<xsl:attribute name="class">zone_draggeable</xsl:attribute>
	...
	<xsl:if test="@size_responsive and not(@only_fieldset)">
		<div>
	   	...
	   	<fieldset>
	   		...
	   		<xsl:attribute name="class">   
		  	<xsl:if test="@class_responsive"><xsl:value-of select="@class_responsive"/></xsl:if><xsl:text><![CDATA[ ]]></xsl:text>shadow<xsl:text><![CDATA[ ]]></xsl:text>
	...
	<xsl:if test="not(@size_responsive and not(@only_fieldset))">
		<fieldset>
			<xsl:attribute name="id">   
	   		...
	   		<xsl:attribute name="class">   
				  	<xsl:if test="@size_responsive"><xsl:value-of
						select="@size_responsive" /></xsl:if><xsl:text><![CDATA[ ]]></xsl:text>
				  	<xsl:if test="@class_responsive"><xsl:value-of
						select="@class_responsive" /></xsl:if><xsl:text><![CDATA[ ]]></xsl:text>
				  	<xsl:if test="not(@visible) or not(@visible='true')">hidden</xsl:if>
...
<xsl:template match="fieldsetexpanded_responsive"> 
	   	...
	   	<fieldset>
			...
			<xsl:call-template name="basic_generate_component_responsive">
					<xsl:with-param name="specialClass">
				  	<xsl:if test="@class_responsive"><xsl:value-of select="@class_responsive"/></xsl:if><xsl:text><![CDATA[ ]]></xsl:text>shadow<xsl:text><![CDATA[ ]]></xsl:text>
				  	<xsl:if test="not(@visible) or not(@visible='true')">hidden</xsl:if>
				  	<xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>expand
				  	<xsl:if test="@dataparent">
			 		  	   <xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>collapse
			   		</xsl:if>
					</xsl:with-param>
	   	    ...
	   	    <xsl:if test="@label">
					  <legend>
					  	   <xsl:value-of select="@label"/>
					  	   <div class="panel-info" style="float:right">
					  	   <xsl:if test="@label_right">
				         	  <xsl:value-of select="@label_right"/>
				         	</xsl:if>
				         	<a>
								 <xsl:attribute name="href">javascript:toogleExpandedFieldset('<xsl:value-of select="@name"/>');
								 </xsl:attribute>
			    				<div>
	...
	<xsl:if test="not(@size_responsive and not(@only_fieldset))">
		...
		<script>
			showExpandedFieldset('<xsl:value-of select="@name"/>');
		<xsl:if test="@onstart-collapse='true'">
				toogleExpandedFieldset('<xsl:value-of select="@name"/>');
		</xsl:if>
		</script>
</xsl:template>
 	...
 	<xsl:variable name="fullname">dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@name"/></xsl:variable>
      <div>
			 ...
			 <input type="hidden">
					...
					<xsl:attribute name="value"><xsl:value-of select="@tab_select"/></xsl:attribute>
				</input>
        ...
        <div>
          <xsl:attribute name="class">
		    ...
		    <xsl:if test="@label_lateral">
			      ...
			      <label>
			        	<xsl:attribute name="for"><xsl:value-of select="$fullname"/></xsl:attribute>
				 			  <xsl:if test="@label_class"><xsl:attribute name="class"><xsl:value-of select="@label_class"/></xsl:attribute></xsl:if>
			        	<xsl:value-of select="@label_lateral"></xsl:value-of>
			      </label>
		      </xsl:if>
			    <ul>
						<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_ul</xsl:attribute>
						<xsl:if test="/page/header/savehelp">			
					    <xsl:attribute name="oncontextmenu">
						      return goToSaveHelp('<xsl:value-of select="@name"/>',event.x,event.y,'<xsl:value-of select="/page/header/savehelp/@win"/>','<xsl:value-of select="/page/header/savehelp/@action"/>','<xsl:value-of select="/page/header/savehelp/@status"/>',0);
							</xsl:attribute>
				  ...
				  <div class="tab-content" >
					...
					<script>
							registerMeta('<xsl:value-of select="$fullname"/>',['<xsl:value-of select="$fullname"/>']);
							selectJSTab('<xsl:value-of select="$fullname"/>');
	      ...
	      <li>
	      	...
	      	<a data-toggle="tab">
	      		<xsl:attribute name="href">#<xsl:value-of select="@id"/></xsl:attribute>
		      		<xsl:attribute name="onclick">
		      			<xsl:if test="@ondemand='true'">
		      				selectTab('#<xsl:value-of select="$fullname"/>','<xsl:value-of select="@id"/>');
		      			...
		      			<xsl:if test="@ondemand!='true'">
			      			if (!selectTab('#<xsl:value-of select="$fullname"/>','<xsl:value-of select="@id"/>')) {
			      				<xsl:for-each select="link">
									<xsl:for-each select="action">
											<xsl:call-template name="generate_action_invokation"/>
									</xsl:for-each>
...
<xsl:template match="zone">
	<DIV>
	  	...
	  	<xsl:if test="@class_responsive"><xsl:attribute name="class"><xsl:value-of select="@class_responsive"/>
	  			  	<xsl:if test="@only-expanded"><xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>collapsable</xsl:if>
					<xsl:if test="@only-collapsed"><xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>nocollapsable</xsl:if>
	...
	<xsl:if test="@affix">
		<script>
			affixObject('<xsl:value-of select="@name"/>');			
		</script>
	</xsl:if>
</xsl:template>
...
<xsl:template match="date_chooser_responsive">
	...
	<xsl:variable name="fullnamecontrol">dgf_<xsl:value-of select="@form_name"/>_fd_<xsl:value-of select="@name"/>_control</xsl:variable>
            <div>
            	<xsl:attribute name="class">
		           	<xsl:choose>
						<xsl:when test="@visible and (@visible='true')">
							<xsl:value-of select="@size_responsive"/><![CDATA[ ]]>form-group
							<xsl:if test="constraints/@required='true'"><![CDATA[ ]]>required</xsl:if>
						</xsl:when>
						<xsl:otherwise>form-group hidden</xsl:otherwise>
					</xsl:choose>
					<xsl:if test="@only-expanded"><xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>collapsable</xsl:if>
		    ...
		    <div>
		        <xsl:attribute name="class">
	 		   			<xsl:if test="@field_class"><xsl:value-of select="@field_class"/></xsl:if>
	 		   		</xsl:attribute>
			      <div>
	            	...
	            	<xsl:attribute name="id"><xsl:value-of select="$fullnamecontrol"/></xsl:attribute>    
	             <input type='text'>
	 							<xsl:attribute name="class">
		 							form-control input-sm
		 							<xsl:if test="@enteristab"><![CDATA[ ]]>enteristab</xsl:if>
		 							<![CDATA[ ]]>focuseable
		 						</xsl:attribute>
						...
						<xsl:if test="@visible and (@visible='true') and @editable='true'">
		                    <span class="input-group-addon">
		                               	<xsl:attribute name="onclick">
			                        	      openDataTimePicker('<xsl:value-of select="$fullnamecontrol"/>');
 			                        	</xsl:attribute>
		                    ...
		                    <xsl:if test="constraints/@required!='true'">
			                    <span class="input-group-addon">
			                        	<xsl:attribute name="onclick">$('#<xsl:value-of select="$fullname"/>').val('');<xsl:if test="@detect_changes='true'">setChangeInputs(true);</xsl:if></xsl:attribute>
             ...
             initializeDataTimePicker('<xsl:value-of select="$fullname"/>'<xsl:if test="@options">,{<xsl:value-of select="@options"/>}</xsl:if>);
        </script>
	<xsl:call-template name="basic_generate_form_component_scripts_out"/>
</xsl:template>
...
<xsl:template match="interval_date_chooser_responsive">
	...
	<xsl:variable name="fullnamecontrol">dgf_<xsl:value-of select="@form_name"/>_fd_<xsl:value-of select="@name"/>_ctrl</xsl:variable>
            <div>
		         ...
		         <div>
		         	 <xsl:attribute name="class">
	 		   		 ...
	 		   		 <xsl:if test="@propname_from!=''">
	 		   		     <input type='hidden'>
	                    	<xsl:attribute name="id"><xsl:value-of select="$fullnamefrom"/></xsl:attribute>
						 </input>
					 </xsl:if>
					 <xsl:if test="@propname_to!=''">
	 		   		     <input type='hidden'>
	                    	<xsl:attribute name="id"><xsl:value-of select="$fullnameto"/></xsl:attribute>
						 </input>
					 </xsl:if>
	 		   		 <div>
	                    ...
	                    <input type='text'>
							...
							<xsl:if test="@needenter='false'">
								<xsl:attribute name="onKeydown">
									if (isClosedIntervalDataTimePicker('<xsl:value-of select="$fullnamecontrol"/>')) {
						...
						<xsl:if test="@label_lateral">
			         	...
			         	<xsl:if test="@visible and (@visible='true') and @editable='true'">
		                    <span class="input-group-addon">
	                               	<xsl:attribute name="onclick">
		                        	      openIntervalDataTimePicker('<xsl:value-of select="$fullnamecontrol"/>');
		                        	</xsl:attribute>
		                    ...
		                    <xsl:if test="constraints/@required!='true'">
			                    <span class="input-group-addon">
			                        	<xsl:attribute name="onclick">
			                        	      clearIntervalDataTimePicker('<xsl:value-of select="$fullnamecontrol"/>','<xsl:value-of select="$fullnamefrom"/>','<xsl:value-of select="$fullnameto"/>',<xsl:value-of select="@detect_changes"/>);
       		...
       		<script charset="ISO-8859-1">
               initializeIntervalDataTimePicker('<xsl:value-of select="$fullname"/>','<xsl:value-of select="$fullnamefrom"/>','<xsl:value-of select="$fullnameto"/>',{<xsl:value-of select="@options"/>},'<xsl:value-of select="@out_format"/>',<xsl:value-of select="@detect_changes"/>);
        </script>
	<xsl:call-template name="basic_generate_form_component_scripts_out"/>
	<xsl:if test="@two_prop='true'">
       		<script charset="ISO-8859-1">
				registerNoConstraint('<xsl:value-of select="$fullnamefrom"/>',['<xsl:value-of select="$fullnamefrom"/>']);
				registerNoConstraint('<xsl:value-of select="$fullnameto"/>',['<xsl:value-of select="$fullnameto"/>']);
		    </script>
	</xsl:if>
</xsl:template>
...
<xsl:template match="dropdowncombo_responsive_noform">
       <xsl:call-template name="dropdowncombo_responsive_basic">
			<xsl:with-param name="inForm" select="'false'" />
       </xsl:call-template>
...
<xsl:template match="dropdowncombo_responsive">
	...
	<xsl:variable name="onclick_event_var"><xsl:value-of select="@onclick_event"/></xsl:variable>
 	     <div>
	 		<xsl:if test="/page/header/savehelp">			
           	...
           	<xsl:attribute name="class">
	           	<xsl:choose>
					<xsl:when test="@visible and (@visible='true')">
						<xsl:value-of select="@size_responsive"/><![CDATA[ ]]>form-group 
						<xsl:if test="@required='true'"><![CDATA[ ]]>required</xsl:if>
					</xsl:when>
					<xsl:otherwise>form-group hidden</xsl:otherwise>
				</xsl:choose>
				  	<xsl:if test="@only-expanded"><xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>collapsable</xsl:if>
	        ...
	        <xsl:call-template name="dropdowncombo_responsive_basic">
	        	<xsl:with-param name="inForm" select="'true'" />
       		</xsl:call-template>
	     </div>
     	<script>
			<xsl:if test="@connect_control">
				subscribeControlConnect(
				'<xsl:value-of select="@connect_control"/>',
				function() {
					hideSelectMetaItemsCombo('<xsl:value-of select="$fullname"/>','<xsl:value-of select="@connect_control_field"/>','dgf_form_<xsl:value-of select="@connect_control"/>_fd-swap_zone','<xsl:value-of select="@connect_control_operator"/>','<xsl:value-of select="@connect_control_datatype"/>');
					hideSelectMetaItemsCombo('<xsl:value-of select="$fullname"/>','<xsl:value-of select="@connect_control_field"/>','dgf_form_<xsl:value-of select="@connect_control"/>_fd-swap_zone_to','<xsl:value-of select="@connect_control_operator"/>','<xsl:value-of select="@connect_control_datatype"/>');
				});
			</xsl:if>
		</script>
...
<xsl:template name="dropdowncombo_responsive_basic">
	<xsl:param name="inForm" select="'true'" />
	<xsl:variable name="fullname">dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@name"/></xsl:variable>
	<xsl:variable name="onchange_event_var"><xsl:value-of select="@onchange_event"/></xsl:variable>
	<xsl:variable name="onclick_event_var"><xsl:value-of select="@onclick_event"/></xsl:variable>
	<xsl:choose>
...
<xsl:template name="dropdowncombo_responsive_internal">
	...
	<div>
	   	<xsl:attribute name="class">
			<xsl:if test="@field_class and (@label_lateral or labelicon)"><xsl:value-of select="@field_class"/></xsl:if>
	   	</xsl:attribute>
		<div>
			...
			<input type="hidden">
	    	   	<xsl:attribute name="id"><xsl:value-of select="$fullname"/></xsl:attribute>
	        	<xsl:attribute name="value"><xsl:value-of select="combo/selected/@value"/></xsl:attribute>
			...
			<input type="text">
			  	<xsl:attribute name="class">
				    	<xsl:if test="$inForm='true'">
				    		form-control
				    	</xsl:if>
				    	<![CDATA[ ]]>input-sm
				    	<![CDATA[ ]]>dropdown-toggle
					<xsl:if test="@class_responsive"><xsl:value-of select="@class_responsive"/></xsl:if>
		    	</xsl:attribute>
		    	<xsl:if test="combo/@editable='true'">
					<xsl:attribute name="onclick">
				  		$('span[id="<xsl:value-of select="$fullname"/>_button"]').click();
					</xsl:attribute>
					<xsl:attribute name="onkeydown">
				  		$('span[id="<xsl:value-of select="$fullname"/>_button"]').click();
					</xsl:attribute>
		    	</xsl:if>
	    	   	<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_data</xsl:attribute>
	    	   	<xsl:attribute name="value"><xsl:value-of select="combo/selected/@description"/></xsl:attribute>
				<xsl:attribute name="readonly">readonly</xsl:attribute>
	
				<xsl:attribute name="style">
					<xsl:call-template name="generate_inline_style_responsive">
							<xsl:with-param name="style_attr">
								<xsl:if test="combo/@editable='true' and @required!='true'">
							  		background:#fff;
								</xsl:if>
							</xsl:with-param>
	        ...
	        <div class="dropdown-menu">	
			   	<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_dropdown</xsl:attribute>
		        <div class="input-group input-group-sm">
				  <span class="input-group-addon">
			    	   	<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_sizing-addon</xsl:attribute>
				  		<i class="fa fa-search" style="float:left"/>
				  </span>
				  <input type="text" class="search form-control" autocomplete="off">
			    	   	<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_search</xsl:attribute>
			    	   	<xsl:attribute name="aria-describedby"><xsl:value-of select="$fullname"/>_sizing-addon</xsl:attribute>
						<xsl:attribute name="oninput">
							javascript:dropdowncombofilter('<xsl:value-of select="$fullname"/>');
						</xsl:attribute>
				...
				<div>
					<xsl:attribute name="style">
						max-height:<xsl:value-of select="@size_rows*20"/>px;overflow:auto;margin-bottom:10px;
					</xsl:attribute>
			        <ul>
				       	<xsl:for-each select="combo/item">
							<li>
								<a>
									...
									<xsl:attribute name="data-real_id"><xsl:value-of select="@real_id"/></xsl:attribute>
			...
			<xsl:if test="combo/@editable='true'">
		        <span role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
					<xsl:attribute name="class">
						input-group-addon
						<![CDATA[ ]]>dropdown-toggle
					  	<xsl:if test="combo/@enteristab"><![CDATA[ ]]>enteristab</xsl:if>
						<![CDATA[ ]]>focuseable
					</xsl:attribute>
	...
	<xsl:call-template name="basic_generate_form_component_scripts_out"/>
	<script>
		dropdowncomboinitialize('<xsl:value-of select="$fullname"/>');
	</script>
</xsl:template>
     	...
     	<script>
			<xsl:if test="@connect_control">
				...
				function() {
					...
					hideSelectMetaItemsLov('<xsl:value-of select="$fullname"/>','<xsl:value-of select="@connect_control_field"/>','dgf_form_<xsl:value-of select="@connect_control"/>_fd-swap_zone','<xsl:value-of select="@connect_control_operator"/>','<xsl:value-of select="@connect_control_datatype"/>');
					hideSelectMetaItemsLov('<xsl:value-of select="$fullname"/>','<xsl:value-of select="@connect_control_field"/>','dgf_form_<xsl:value-of select="@connect_control"/>_fd-swap_zone_to','<xsl:value-of select="@connect_control_operator"/>','<xsl:value-of select="@connect_control_datatype"/>');
...
<xsl:template name="dropdownwinlov_responsive_internal">
	...
	<div>
		...
		<input type="hidden">
    	   	<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_isopen</xsl:attribute>
        	<xsl:attribute name="value"><xsl:value-of select="@opened"/></xsl:attribute>
		</input>	    			
		<input type="hidden">
    	   	<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_zobject</xsl:attribute>
        	<xsl:attribute name="value"><xsl:value-of select="@zobject"/></xsl:attribute>
		...
		<input type="hidden">
    	   	<xsl:attribute name="id"><xsl:value-of select="$fullname"/></xsl:attribute>
        	<xsl:attribute name="value"><xsl:value-of select="winlov/item/@id"/></xsl:attribute>
			<xsl:call-template name="basic_generate_form_component_scripts_in">
				<xsl:with-param name="onchange_event_var"><xsl:value-of select="$onchange_event_var"/></xsl:with-param>
				<xsl:with-param name="onclick_event_var"><xsl:value-of select="$onclick_event_var"/></xsl:with-param>
			</xsl:call-template>
		...
		<input type="text">
		  	<xsl:attribute name="class">
    	   	...
    	   	<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_data</xsl:attribute>
    	   	<xsl:attribute name="value"><xsl:value-of select="winlov/item/@description"/></xsl:attribute>
			<xsl:attribute name="readonly">readonly</xsl:attribute>

			<xsl:attribute name="style">
				<xsl:call-template name="generate_inline_style_responsive">
						<xsl:with-param name="style_attr">
							<xsl:if test="winlov/@editable='true' and @required!='true'">
						  		background:#fff;
							</xsl:if>
						</xsl:with-param>
        ...
        <div class="modal modal-component" role="dialog" data-backdrop="false" >	
		   	<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_dropdown</xsl:attribute>
		    <div class="modal-dialog-component" >
		   		<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_dialog</xsl:attribute>
		...
		<xsl:for-each select="action_edit">
			<span>
				<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_view</xsl:attribute>
				<xsl:attribute name="class">
					input-group-addon
					<![CDATA[ ]]>dropdown-toggle
				  	<xsl:if test="winlov/@enteristab"><![CDATA[ ]]>enteristab</xsl:if>
					<![CDATA[ ]]>focuseable
				</xsl:attribute>
				<xsl:attribute name="onclick">
						<xsl:for-each select="action">
								<xsl:call-template name="generate_action_invokation">
									<xsl:with-param name="dynamic_owner"><xsl:value-of select="$fullname"/></xsl:with-param>
								</xsl:call-template>
						</xsl:for-each>
				</xsl:attribute>
		...
		<xsl:if test="winlov/@editable='true'">
	        <span role="button" data-toggle="modal">
				<xsl:attribute name="data-target">
					#<xsl:value-of select="$fullname"/>_dropdown
				</xsl:attribute>
				<xsl:attribute name="class">
					input-group-addon
					<![CDATA[ ]]>dropdown-toggle
	   		...
	   		<span role="button"> 
				<xsl:attribute name="class">input-group-addon</xsl:attribute>
					<xsl:attribute name="onclick">
						dropdownwinlovclear('<xsl:value-of select="$fullname"/>');
					</xsl:attribute>
		
					<span><xsl:attribute name="class">glyphicon glyphicon-remove</xsl:attribute></span>
	...
	<xsl:call-template name="basic_generate_form_component_scripts_out"/>
	<script>
		registerNoConstraint('<xsl:value-of select="$fullname"/>_isopen',['<xsl:value-of select="$fullname"/>_isopen']);
		registerNoConstraint('<xsl:value-of select="$fullname"/>_zobject',['<xsl:value-of select="$fullname"/>_zobject']);
		dropdownWinlovinitialize('<xsl:value-of select="$fullname"/>',<xsl:value-of select="@is_open"/>,<xsl:value-of select="winlov/@editable"/>,'<xsl:value-of select="@form_objectOwner"/>','<xsl:value-of select="@form_objectAction"/>','<xsl:value-of select="@form_objprovider"/>',<xsl:value-of select="winlov/@show_key"/>);
	</script>
</xsl:template>
					...
					<button>
						...
						<xsl:if test="@title">
					           <div>
					           		<xsl:if test="icon">
					           			<xsl:call-template name="render_icon">
											     <xsl:with-param name="style_attr">padding-right: 5px;<xsl:if test="@style_image"><xsl:value-of select="@style_image"/></xsl:if></xsl:with-param>
					           			</xsl:call-template>	
						            </xsl:if>
						            ...
						            <xsl:choose>
										...
										<xsl:otherwise>
								            <div>
									           <xsl:if test="@class_position_icon and icon"><xsl:attribute name="class"><xsl:value-of select="@class_position_icon"/></xsl:attribute></xsl:if>
									           <xsl:if test="@title"><strong><xsl:value-of select="@title"/></strong></xsl:if>
									           <xsl:if test="@subtitle" >
									           		<span class="pull-right text-muted">
							                             <em><xsl:value-of select="@subtitle"/></em>
				               ...
				               <div ><xsl:value-of select="@label" disable-output-escaping="yes"/></div>
				        </xsl:if>
						...
						<xsl:if test="not(@title) and not(@label)">
				           		<xsl:if test="icon">
				           			<xsl:call-template name="render_icon">
										     <xsl:with-param name="style_attr"><xsl:if test="@style_image"><xsl:value-of select="@style_image"/></xsl:if></xsl:with-param>
				           			</xsl:call-template>	
					            </xsl:if>
				        </xsl:if>
					</button>
					...
					<xsl:if test="@title">
				           <div>
					            ...
					            <xsl:choose>
									...
									<xsl:otherwise>
						            <div >           
							            <xsl:if test="@class_position_icon"><xsl:attribute name="class"><xsl:value-of select="@class_position_icon"/></xsl:attribute></xsl:if>
							            <xsl:if test="@title"><strong><xsl:value-of select="@title"/></strong></xsl:if>
							            <xsl:if test="@subtitle">
							           		<span class="pull-right text-muted">
					                             <em><xsl:value-of select="@subtitle"/></em>
			...
			<xsl:choose>
	    		<xsl:when test="@action">
	    			...
	    			<xsl:if test="@confirmation">
						  <xsl:attribute name="href">javascript:runAction(this,true,'<xsl:value-of select="@confirmation"/>',null, function(){
			    			<xsl:choose>
			    				<xsl:when test="@opens_new_window and @opens_new_window='true'">
									goToMenuActionResponsive('<xsl:value-of select="@action"/>','true');
			    				</xsl:when>
								<xsl:otherwise><!-- not(@opens_new_window and @opens_new_window='true') -->
									goToMenuActionResponsive('<xsl:value-of select="@action"/>','false');
								</xsl:otherwise>	    			
		    				</xsl:choose>
		    				},function() {});
		    			 </xsl:attribute>
...
<xsl:template match="navigation_group">
	 <li>
    	...
    	<xsl:if test="@data_toggle">
		  	<xsl:attribute name="class"><xsl:value-of select="@nav_group_class"/></xsl:attribute>
    	</xsl:if>
	    <a>
...
<xsl:template match="navigation_group_sidebar">
          <div role="navigation">
			  	<xsl:if test="@name"><xsl:attribute name="id"><xsl:value-of select="@name"/></xsl:attribute></xsl:if>
			  	<xsl:if test="@class_navigation"><xsl:attribute name="class"><xsl:value-of select="@class_navigation"/></xsl:attribute></xsl:if>
                <div>
				  	<xsl:if test="@name"><xsl:attribute name="id"><xsl:value-of select="@name"/>_sidebar</xsl:attribute></xsl:if>
				  	<xsl:if test="@class_navigation_internal"><xsl:attribute name="class"><xsl:value-of select="@class_navigation_internal"/></xsl:attribute></xsl:if>
                    <ul>
									  	<xsl:if test="@class_responsive"><xsl:attribute name="class"><xsl:value-of select="@class_responsive"/></xsl:attribute></xsl:if>
									  	<xsl:if test="@role"><xsl:attribute name="role"><xsl:value-of select="@role"/></xsl:attribute></xsl:if>
										<xsl:call-template name="basic_generate_component_responsive"/>
...
<xsl:template match="break">
	<xsl:choose>
		...
		<xsl:when test="not(@appled_by_layout='true')"><br></br></xsl:when>
	</xsl:choose>
</xsl:template>
...
<xsl:template match="image_responsive">
	<xsl:choose>
		<xsl:when test="@sinDIV">
			<xsl:call-template name="view_image_responsive"></xsl:call-template>
		</xsl:when>
		<xsl:otherwise>
			<div>
				<xsl:call-template name="basic_generate_component_responsive">
...
<xsl:template match="dropdown">
    ...
    <div>
	    ...
	    <button type="button" data-toggle="dropdown">
			<xsl:if test="@class_button"><xsl:attribute name="class"><xsl:value-of select="@class_button"/></xsl:attribute></xsl:if>
			...
			<span>
				<xsl:if test="arrow/icon/@file"><xsl:attribute name="class"><xsl:value-of select="arrow/icon/@file"/></xsl:attribute></xsl:if>
			</span>
		</button>
...
<xsl:template match="dropdown_responsive">
	<xsl:variable name="fullname">dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@name"/></xsl:variable>
		  <div>
           	<xsl:attribute name="class">
      		   <xsl:choose>
					<xsl:when test="@visible and (@visible='true')"><xsl:if test="@size_responsive">
						<xsl:value-of select="@size_responsive"/></xsl:if><![CDATA[ ]]>form-group
\					</xsl:when>
					<xsl:otherwise>form-group hidden</xsl:otherwise>
			   </xsl:choose>
			   				  	<xsl:if test="@only-expanded"><xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>collapsable</xsl:if>
		    ...
		    <div>
			    ...
			    <button type="button" data-toggle="dropdown">
					...
					<xsl:if test="@class_button"><xsl:attribute name="class"><xsl:value-of select="@class_button"/></xsl:attribute></xsl:if>
					<span class="pull-left"><xsl:value-of select="@label_button"/>
					<xsl:if test="@image">
						<i>
							<xsl:attribute name="class"><xsl:value-of select="@image"/></xsl:attribute>
			       		</i>		
					...
					<span>
						<xsl:if test="arrow/icon/@file"><xsl:attribute name="class">pull-right <xsl:value-of select="arrow/icon/@file"/></xsl:attribute></xsl:if>
					</span>
				</button>
...
<xsl:template name="win_list_table">
	    <xsl:param name="level" select="'_base'" />
			...
			<script>
 	 		if ($('#actionbarinternal_<xsl:value-of select="@obj_provider"/>').html()!="") {
		     $("#<xsl:value-of select="@obj_provider"/>_toolbar").html($('#actionbarinternal_<xsl:value-of select="@obj_provider"/>').html());
			   $('#actionbarinternal_<xsl:value-of select="@obj_provider"/>').html(""); 
		    }
			 <xsl:if test="@modifiedonserver='true'">
...
<xsl:template name="tree_table_responsive">
	...
	<script>
		$(document).ready(function() {
	        $("#<xsl:value-of select="@obj_provider"/>_table").treeFy({
	            expanderTemplate:'<xsl:value-of select="@expanderTemplate"/>',	<!-- 	HTML Element when you click on that will be collapse/expand branches -->
					indentTemplate:'<xsl:value-of select="@indentTemplate"/>',		<!-- 	HTML Element that will be placed as padding, depending on the depth of nesting node -->
					expanderExpandedClass:'<xsl:value-of select="@expanderExpandedClass"/>',						<!-- 	Class using for expander element when it expanded -->
					expanderCollapsedClass:'<xsl:value-of select="@expanderCollapsedClass"/>',						<!-- 		Class using for expander element when it collapsed -->
					treeColumn:	<xsl:value-of select="@treeColumn"/>,													<!-- 	Index of the column which expander should be added to. 0-based. -->
					initStatusClass: '<xsl:value-of select="@initStatusClass"/>', 										 	<!-- 		Initial state of tree -->
																					<!-- 	'treetable-expanded' - all nodes will be expanded -->
																					<!-- 	'treetable-collapsed' - all nodes will be collapsed -->
					collapseAnimateCallback:function(row){row.hide();},				<!-- 		create custom animations, triggered when collapsing -->
					expandAnimateCallback:function(row){row.show();}				<!-- 	create custom animations, triggered when expanding -->
	        });
	...
	<!-- <div>
		<xsl:attribute name="id"><xsl:value-of select="@obj_provider"/>_toolbar</xsl:attribute>
	</div>-->
	<div>
		<xsl:call-template name="generate_table">
			<xsl:with-param name="level"><xsl:value-of select="$level"/></xsl:with-param>
...
<xsl:template name="generate_table">
	   ...
	   <div class="table-responsive">
	    ...
	    <xsl:attribute name="id"><xsl:value-of select="@obj_provider"/>_container</xsl:attribute>
	    <table>
				<xsl:attribute name="class"><xsl:value-of select="@class_table_responsive"/></xsl:attribute>
				...
				<xsl:if test="$level='_base'">
				   ...
				   <xsl:if test="@zoomtofit">
						<xsl:attribute name="style">
			   			width:<xsl:value-of select="@zoomtofit_width"/>px;
			   		</xsl:attribute>    				
				...
				<tr>
					<xsl:for-each select="header">
						...
						<xsl:for-each select="column">
							<th>								
								<xsl:variable name="columnname"><xsl:value-of select="../../@obj_provider"/>_<xsl:value-of select="position()"/></xsl:variable>
								<xsl:attribute name="id"><xsl:value-of select="$columnname"/></xsl:attribute>
								...
								<xsl:if test="@is_html='true'">
									<xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>column-html
								</xsl:if>
								...
								<xsl:if test="../../@allowSortedColumns='true' and not(../../@with_group='true')">
									<span>
										<xsl:attribute name="id"><xsl:value-of select="$columnname"/>_resize</xsl:attribute>
										...
										<i class="fa fa-grip-vertical" style="color:#00000052"/>	
									</span>
								</xsl:if>
								<xsl:choose>
								...
								<script type="text/javascript" charset="ISO-8859-1">
									$("#<xsl:value-of select="$columnname"/>").resizable({
										resizeHeight : false
									});
				...
				<TBODY id="scroll_body">
	 			<xsl:for-each select="rows/row">
					...
					<tr>
						<xsl:attribute name="rowpos"><xsl:value-of select="@rowpos"/></xsl:attribute>
						...
						<xsl:if test="../../@is_line_select='true' and ../../@use_context_menu='true'">
	      				...
	      				<xsl:if test="@rowpos mod 2 = 0"><xsl:attribute name="class">odd</xsl:attribute></xsl:if>
	      				<xsl:if test="@rowpos mod 2 = 1"><xsl:attribute name="class">even</xsl:attribute></xsl:if>
						...
						<xsl:if test="@id_tree"><xsl:attribute name="data-node"><xsl:value-of select="@id_tree"/></xsl:attribute></xsl:if>
						<xsl:if test="@id_tree_parent"><xsl:attribute name="data-pnode"><xsl:value-of select="@id_tree_parent"/></xsl:attribute></xsl:if>
						<xsl:if test="$level='_base'">
							<xsl:call-template name="dragdrop_component"/>
							<xsl:if test='smplclick'>
								<xsl:if test="../../@has_action_bar='true'">
									<xsl:choose>
										<xsl:when test="@key">
												<xsl:attribute name="onclick">
												return dc6(this,event,function(that) { 
														<xsl:call-template name="generate_action_invokation">
										...
										<xsl:otherwise>
											<xsl:if test="../../@is_line_select='true'">
												<xsl:attribute name="onclick">
													return dc5(this,event,function(that) { 
														<xsl:call-template name="generate_action_invokation">
															<xsl:with-param name="that">that</xsl:with-param> 
														</xsl:call-template>
													});
							...
							<xsl:if test="not(@visible) or @visible='true'">
								<xsl:if test="not(@skip) or not(@skip='true')">
									...
									<xsl:call-template name="generate_win_list_cell">
										...
										<xsl:with-param name="rowpos"><xsl:value-of select="../@rowpos"/></xsl:with-param>
									</xsl:call-template>	
								</xsl:if>
							</xsl:if>
						</xsl:for-each>
			   ...
			   <script>
				...
				<xsl:for-each select="actions/action">
					...
					<xsl:if test="not_allowed_win">
						AAANodd('<xsl:value-of select="@id"/>',<xsl:value-of select="@win"/>,[<xsl:for-each select="not_allowed_win">'<xsl:value-of select="@id"/>',</xsl:for-each>null],<xsl:value-of select="@multiple"/>);
					</xsl:if>
				</xsl:for-each>
				...
				initializeTable('<xsl:value-of select="@obj_provider"/>_table','<xsl:value-of select="@obj_provider"/>_container',<xsl:value-of select="@is_line_select"/>);
	    	    <xsl:for-each select="default_actions">
					    <xsl:for-each select="default_action">
					        <xsl:choose>
					        	<xsl:when test="@name='dblclick'">
							    	$('#<xsl:value-of select="../../@obj_provider"/>_table tbody').off('dblclick');
							    	$('#<xsl:value-of select="../../@obj_provider"/>_table tbody').off('keydown');
							    	$('#<xsl:value-of select="../../@obj_provider"/>_table tbody').off('click');
							    	$('#<xsl:value-of select="../../@obj_provider"/>_table tbody').on('dblclick','tr', function() {
										<xsl:for-each select="action">
							    	...
							    	$('#<xsl:value-of select="../../@obj_provider"/>_table tbody').on('keydown','tr', function() {
										<xsl:for-each select="action">
					        	...
					        	<xsl:when test="@name='smplclick'">
							    	$('#<xsl:value-of select="../../@obj_provider"/>_table tbody').on('click','tr', function() {
										<xsl:for-each select="action">
											return dc5(this,event,function(that) { 
												<xsl:call-template name="generate_action_invokation">
													<xsl:with-param name="that">that</xsl:with-param> 
	  ...
	  <xsl:if test="$level='_base'">
		   <xsl:if test="@zoomtofit">
				<script >
				   zoomToFitHorizontal('<xsl:value-of select="@obj_provider"/>_container','<xsl:value-of select="@obj_provider"/>_table');
				   $(window).off('<xsl:value-of select="@obj_provider"/>_container');
				   $(window).resize('<xsl:value-of select="@obj_provider"/>_container',function(){
				 		zoomToFitHorizontal('<xsl:value-of select="@obj_provider"/>_container','<xsl:value-of select="@obj_provider"/>_table');
				   });
				</script>  
			 </xsl:if>
...
<xsl:template name="win_list_table_bigdata">
		...
		<script>
			$(document).ready(function(){
					...
					var focusName=null;
		 		    var table = $('#<xsl:value-of select="@obj_provider"/>_table').on('xhr.dt', function ( e, settings, json, xhr ) {
		 		    			if (json==null) return;
				               	<xsl:for-each select="header/updateControl">
				               		updateField($('input[id*="<xsl:value-of select="@field"/>"]').prop('id'),json.<xsl:value-of select="@data"/>);
								</xsl:for-each>
					    } ).DataTable( {
				        "destroy": true,
				        "responsive": <xsl:value-of select="@list_responsive"/>,
				        "processing": true,
				        "dom": '<xsl:value-of select="@distribution"/>',
				        "serverSide": true, 
            	        "ordering": false,
				        "searching": true,
					    "scrollY":  550,
				        "scroller": {
				          ...
				          "preDrawCallback": function (settings) {
						    onFocusObjectById($(':focus').attr('id'));
					        if (!(typeof table === 'undefined')) 
					          if(table!=null) {
					         <xsl:for-each select="header/manualFilter">
			         			<xsl:for-each select="column">
			         			    <xsl:if test ="not(@name=preceding::column/@name)">
				         				var <xsl:value-of select="@name"/>Data="";
									</xsl:if>
					            </xsl:for-each>
				            </xsl:for-each>					
			               	<xsl:for-each select="header/manualFilter">
			               		<xsl:choose>
				               		<xsl:when test="@control='JFormCheckResponsive'">
										if ($( 'input[type="checkbox"][id*="<xsl:value-of select="@field"/>"]' ).length>0)
										 if ( $( 'input[type="checkbox"][id*="<xsl:value-of select="@field"/>"]' ).get(0).checked) {
										<xsl:for-each select="column">
											<xsl:value-of select="@name"/>Data+=((<xsl:value-of select="@name"/>Data=='')?'':';')+ '<xsl:value-of select="../@field"/>|<xsl:value-of select="../@oper"/>|'+$( 'input[type="checkbox"][id*="<xsl:value-of select="../@field"/>"]' ).get(0).checked ;
						                </xsl:for-each>
						                }
				               		</xsl:when>
				               		<xsl:when test="@control='JFormWinLOVResponsive'">
					     					if ($( 'select[id*="<xsl:value-of select="@field"/>"]' ).length>0) 
					     					 if ( getSelectValues($( 'select[id*="<xsl:value-of select="@field"/>"]' ).get(0)) !='') {
						     				<xsl:for-each select="column">
							             		<xsl:value-of select="@name"/>Data+=((<xsl:value-of select="@name"/>Data=='')?'':';')+  '<xsl:value-of select="../@field"/>|<xsl:value-of select="../@oper"/>'+'|'+getSelectValues($( 'select[id*="<xsl:value-of select="../@field"/>"]' ).get(0)) ;
							                </xsl:for-each>
							                }
                
				               		</xsl:when>
				               		<xsl:otherwise>
				               				if ( $( 'input[id*="<xsl:value-of select="@field"/>"]' ).length>0) 
				               				 if( $( 'input[id*="<xsl:value-of select="@field"/>"]' ).get(0).value!='') {
												<xsl:for-each select="column">
													<xsl:value-of select="@name"/>Data+=((<xsl:value-of select="@name"/>Data=='')?'':';')+  '<xsl:value-of select="../@field"/>|<xsl:value-of select="../@oper"/>'+'|'+$( 'input[id*="<xsl:value-of select="../@field"/>"]' ).get(0).value ;
								                </xsl:for-each>
							                }
				               		</xsl:otherwise>
			               		</xsl:choose>
			               	...
			               	<xsl:for-each select="header/manualFilter">
			         			<xsl:for-each select="column">
			         			    <xsl:if test ="not(@name=preceding::column/@name)">
			         			    	if (table.column('<xsl:value-of select="@name"/>:name').search()!= <xsl:value-of select="@name"/>Data )
											table.column('<xsl:value-of select="@name"/>:name').search( <xsl:value-of select="@name"/>Data );
									</xsl:if>
					            </xsl:for-each>
				            </xsl:for-each>
				            }
						      ...
						      "aoSearchCols": [
									<xsl:for-each select="header/column[@col_name!='']">
										{ "sSearch" : function() {
											<xsl:variable name="col"><xsl:value-of select="@col_name"/></xsl:variable>
									    	var out="";
										   	<xsl:for-each select="../manualFilter[column/@name=$col]">
							               		<xsl:choose>
								               		<xsl:when test="@control='JFormCheckResponsive'">
														if ($( 'input[type="checkbox"][id*="<xsl:value-of select="@field"/>"]' ).length>0)
														 if ( $( 'input[type="checkbox"][id*="<xsl:value-of select="@field"/>"]' ).get(0).checked) {
														<xsl:for-each select="column[@name=$col]">
															 out+=(out==""?'':';')+ '<xsl:value-of select="../@field"/>|<xsl:value-of select="../@oper"/>|'+$( 'input[type="checkbox"][id*="<xsl:value-of select="../@field"/>"]' ).get(0).checked ;
										                </xsl:for-each>
										                }
								               		</xsl:when>
								               		<xsl:when test="@control='JFormWinLOVResponsive'">
									     					if ($( 'select[id*="<xsl:value-of select="@field"/>"]' ).length>0) 
									     					 if ( getSelectValues($( 'select[id*="<xsl:value-of select="@field"/>"]' ).get(0)) !='') {
										     				<xsl:for-each select="column[@name=$col]">
											             		out+=(out==""?'':';')+  '<xsl:value-of select="../@field"/>|<xsl:value-of select="../@oper"/>'+'|'+getSelectValues($( 'select[id*="<xsl:value-of select="../@field"/>"]' ).get(0)) ;
											                </xsl:for-each>
											                }
								               		...
								               		<xsl:otherwise>
								               				if ( $( 'input[id*="<xsl:value-of select="@field"/>"]' ).length>0) 
								               				 if( $( 'input[id*="<xsl:value-of select="@field"/>"]' ).get(0).value!='') {
																<xsl:for-each select="column[@name=$col]">
																	out+=(out==""?'':';')+  '<xsl:value-of select="../@field"/>|<xsl:value-of select="../@oper"/>'+'|'+$( 'input[id*="<xsl:value-of select="../@field"/>"]' ).get(0).value ;
												                </xsl:for-each>
											                }
								               		</xsl:otherwise>
							               		</xsl:choose>
		    	        ...
		    	        "createdRow": function (row, data, dataIndex) {
				            $(row).attr('data-provider', data.DT_RowId);
				            $(row).attr('data-provider_parent', '<xsl:value-of select="@obj_provider"/>');
				        },			 		 	
				        <xsl:for-each select="json">
							<xsl:for-each select="action">
						        "ajax": {
						            url: '<xsl:value-of select="@target"/><xsl:if test="@data_string and not(@data_string='')">?<xsl:value-of select="@data_string"/></xsl:if>',
						            method: 'POST',
						     		data: fillJSonRequest('<xsl:value-of select="@target"/>','<xsl:value-of select="@id_action"/>',<xsl:choose><xsl:when test="@object_owner_id">'<xsl:value-of select="@object_owner_id"/>'</xsl:when><xsl:otherwise>null</xsl:otherwise></xsl:choose>,'<xsl:value-of select="@obj_provider" />')
						        } ,
							</xsl:for-each>
						</xsl:for-each>
						"initComplete": function(settings, json) {
							...
							<xsl:if test="@is_line_select='true'">
								<xsl:for-each select="dblclick">
								    $('#<xsl:value-of select="../@obj_provider"/>_table').on('dblclick', 'tr', function () {
									   	clearClick();
									    rs_forced_with_key(this,'','',event);
										<xsl:for-each select="action">
											<xsl:call-template name="generate_action_invokation"/>
										</xsl:for-each>
										this.disabled=false;
								    ...
								    $('#<xsl:value-of select="@obj_provider"/>_table').on('click', 'tr', function () {
										rs(this,event);
										<xsl:for-each select="smplclick">
											    runClick(this,event,function(that){
														<xsl:for-each select="action">
																<xsl:call-template name="generate_action_invokation"/>
														</xsl:for-each>
												});				           
								...
								$('#<xsl:value-of select="@obj_provider"/>_table').on( 'draw.dt', function () {
									var zTable= getTableRegistered('<xsl:value-of select="@obj_provider"/>_table');
									zTable.updateActionBar();
									zTable.rs_forcedJSON();
								} );

				               	<xsl:for-each select="header/manualFilter">
				               		<xsl:choose>
					               		<xsl:when test="@control='JFormCheckResponsive'">
							                $( 'input[type="checkbox"][id*="<xsl:value-of select="@field"/>"]' ).on( 'change', function () {
											   console.log("change check <xsl:value-of select="@field"/> ");
							                	if (pendingDraw) {
								                	return;
							                	}
											   console.log("change check <xsl:value-of select="@field"/> draw");
							                	pendingDraw=true;
								                table.draw();
							                } );
					               		</xsl:when>
					               		<xsl:when test="@control='JFormWinLOVResponsive'">
							                $( 'select[id*="<xsl:value-of select="@field"/>"]' ).on( 'change select2:unselect select2:select', function () {
												   console.log("change winlov <xsl:value-of select="@field"/> ");
							                	if (pendingDraw) {
								                	return;
							                	}
												   console.log("change winlov <xsl:value-of select="@field"/> draw ");
							                	pendingDraw=true;
								                table.draw();
							                } );
					               		...
					               		<xsl:otherwise>
							                $( 'input[id*="<xsl:value-of select="@field"/>"]' ).on( 'keyup change clear', function () {
												   console.log("change edit <xsl:value-of select="@field"/> ");
							                	if (pendingDraw) {
								                	return;
							                	}
												   console.log("change edit <xsl:value-of select="@field"/> draw");
							                	pendingDraw=true;
								                table.draw();
							                } );
					               		</xsl:otherwise>
							...
							<xsl:if test="@is_line_select='false'">
							    $('#<xsl:value-of select="@obj_provider"/>_table').on('onclick', 'tr', function () {
								   	rsCell(this,event);
							    } );	
								$('#<xsl:value-of select="@obj_provider"/>_table').on( 'draw.dt', function () {
									var zTable= getTableRegistered('<xsl:value-of select="@obj_provider"/>_table');
					    ...
					    "language": {
				            ...
				            "decimal":        ",",
						    "emptyTable":     global_jsondtnodata,
						    "info":           global_jsondtinfo,
						    "infoEmpty":      global_jsondtinfoempty,
						    "infoFiltered":   global_jsondtinfomax,
						    "infoPostFix":    "",
  							"thousands":      ".",
						    "lengthMenu":     global_jsondtlines,
						    "loadingRecords": global_jsondtloading,
						    "search":         '<i class="fa fa-search"/>',
						    "zeroRecords":    global_jsondtzerorecords,
			                "select": {
				                rows: {
				                    _: global_jsondtrowselect,
				                    0: global_jsondtrow0,
				                    1: global_jsondtrow1
				                }
				            }
				         }
				         <xsl:if test="header/column/@col_name">
			                 ,columns: [
					          	<xsl:for-each select="header">
									<xsl:for-each select="column">
										<xsl:if test="@type!='JLINK'">
								            { 
										        "name": '<xsl:value-of select="@col_name"/>',
								            	<xsl:choose>
									            	<xsl:when test="@control='JSCRIPT'">
												        "data": null, 
												        "render": function(data,type,row) { return <xsl:value-of select="@onCalculate"/>}
													</xsl:when>
													<xsl:otherwise>
										            	"data": '<xsl:value-of select="@col_name"/>',
										            	<xsl:choose>
											            	<xsl:when test="@halignment='left'">
												            	"className": "text-left",
											            	</xsl:when>
											            	<xsl:when test="@halignment='right'">
												            	"className": "text-right",
											            	</xsl:when>
											            	<xsl:when test="@halignment='center'">
														...
														<xsl:when test="@control='JINPUT'">
															<xsl:choose>
												            	<xsl:when test="@type='JLONG' or @type='JINTEGER' or @type='JFLOAT'">
												            		"render": function (data, type, row){
												            			return <xsl:text>'&lt;input id=\"'+row.DT_RowId+'_</xsl:text><xsl:value-of select="@col_name"/><xsl:text>\" type=\"text\" value=\"'+data+'\" class=\"</xsl:text><xsl:value-of select="@class_component"/><xsl:text><![CDATA[ ]]>text-right\"  data-colname=\"</xsl:text><xsl:value-of select="@col_name"/><xsl:text>\"/&gt;';</xsl:text>
																	},
																</xsl:when>
																<xsl:otherwise>
												            		"render": function (data, type, row){
												            			return <xsl:text>'&lt;input id=\"'+row.DT_RowId+'_</xsl:text><xsl:value-of select="@col_name"/><xsl:text>\" type=\"text\" value=\"'+data+'\" class=\"</xsl:text><xsl:value-of select="@class_component"/><xsl:text><![CDATA[ ]]>text-left\"  data-colname=\"</xsl:text><xsl:value-of select="@col_name"/><xsl:text>\"/&gt;';</xsl:text>
																	},
																
																</xsl:otherwise>
															</xsl:choose>
														</xsl:when>
														<xsl:when test="@control='JCHECK'">
										            		"render": function (data, type, row){
										            			return <xsl:text>'&lt;input id=\"'+row.DT_RowId+'_</xsl:text><xsl:value-of select="@col_name"/><xsl:text>\" type=\"checkbox\" value=\"S\" class=\"</xsl:text><xsl:value-of select="@class_component"/><xsl:text>\" '+(data=='S'?'checked':'')+' data-colname=\"</xsl:text><xsl:value-of select="@col_name"/><xsl:text>\"/&gt;';</xsl:text>
															},
														</xsl:when>
														<xsl:when test="@type='JIMAGE'">
										            		"render": function (data, type, row){
															    return (data=='')?<xsl:text>' '</xsl:text>:<xsl:text>'&lt;img src="</xsl:text><xsl:value-of select="$skin_path"/><xsl:text>/'+data+'"/&gt;';</xsl:text>
															},
														</xsl:when>											
										            	<xsl:when test="@type='JBOOLEAN'">
										            		"render": function (data, type, row){
										            			if (data=='S') 
																    return ('<xsl:value-of select="@image_true"/>'=='')?<xsl:text>' '</xsl:text>:<xsl:text>'&lt;img src="</xsl:text><xsl:value-of select="$skin_path"/><xsl:text>/</xsl:text><xsl:value-of select="@image_true"/><xsl:text>"/&gt;';</xsl:text>
										            			return ('<xsl:value-of select="@image_false"/>'=='')?<xsl:text>' '</xsl:text>:<xsl:text>'&lt;img src="</xsl:text><xsl:value-of select="$skin_path"/><xsl:text>/</xsl:text><xsl:value-of select="@image_false"/><xsl:text>"/&gt;';</xsl:text>
															},
														</xsl:when>	
										            	<xsl:when test="@type='JCOLOUR'">
										            		"render": function (data, type, row){
										            			return <xsl:text>'&lt;div style=\"height: 16; width: 16;	border-bottom-width: 1px; border-top-width: 1px; border-right-width: 1px; border-left-width: 1px; border-style: solid solid none; border-top-color: #000000; border-bottom-color: #000000; order-right-color: #000000; border-left-color: #000000;	background-color: #'+data+'\"/&gt;';</xsl:text>
															},
														</xsl:when>	
										            	<xsl:otherwise>
										            		"render": function (data, type, row){
										            			if ((typeof data === 'string' || data instanceof String)) {
											            			if (!data.startsWith("*b64*:")) return data; 
										            				return Base64.decode(data.substring(6));
										            			}
																return '';
															},
														</xsl:otherwise>	
										            	...
										            	"orderable": true,
									                    "searchable": true,
									                    "visible": <xsl:choose><xsl:when test="@visible='true'">true</xsl:when><xsl:otherwise>false</xsl:otherwise></xsl:choose>
													</xsl:otherwise>
												</xsl:choose>
											}<xsl:if test="position()!=last()">,</xsl:if>
										</xsl:if>
							            ...
							            <xsl:if test="@type!='JLINK'">
								            {
										    	...
										    	"targets": '<xsl:value-of select="@col_name"/>',
										    	"visible": <xsl:choose><xsl:when test="@visible='true'">true</xsl:when><xsl:otherwise>false</xsl:otherwise></xsl:choose>,
											    "createdCell": function (td, cellData, rowData, row, col){
													 		if (! rowData.DT_RowData) return;
												         	var backColor = rowData.DT_RowData['<xsl:value-of select="@col_name"/>_background']; 
												         	if (backColor) if(backColor!='') $(td).css("background-color",backColor); 
												         	var cellColor = rowData.DT_RowData['<xsl:value-of select="@col_name"/>_foreground']; 
												         	if (cellColor) if(cellColor!='') $(td).css("color",cellColor); 
												}
											}<xsl:if test="position()!=last()">,</xsl:if> 
										</xsl:if>
									</xsl:for-each>
				    ...
				    if ($('#actionbarinternal_<xsl:value-of select="@obj_provider"/>').html()!="") {
				       $("#<xsl:value-of select="@obj_provider"/>_table_wrapper div.toolbar").html($('#actionbarinternal_<xsl:value-of select="@obj_provider"/>').html());
					   $('#actionbarinternal_<xsl:value-of select="@obj_provider"/>').html("");
				    }
				    $('#<xsl:value-of select="@obj_provider"/>_table').on('keydown', 'input', function () {
				  	   	return gaOnKey(event, this);
				    } );	
				    $('#<xsl:value-of select="@obj_provider"/>_table').on('change', 'input', function () {
				       	updateServerField(this,event,table);
				    } );	
				 });
				 $('a[data-toggle="tab"]').on('shown.bs.tab', function(e){
					$('#<xsl:value-of select="@obj_provider"/>_table').DataTable().ajax.reload();
				} );		
				...
				<tr>
					<xsl:for-each select="header">
						<xsl:for-each select="column">
							<xsl:if test="@type!='JLINK'"> 
								<th>
									<xsl:attribute name="class"><xsl:value-of select="@col_name"/></xsl:attribute>
									<xsl:if test="@foreground or @background">
										<xsl:attribute name="style">
											<xsl:if test="@foreground">color:<xsl:value-of select="@foreground"/>;</xsl:if>
											<xsl:if test="@background">background-color:<xsl:value-of select="@background"/>;</xsl:if>
...
<xsl:template name="win_list_table_json">
		...
		<script>
			$(document).ready(function(){
		 		    var table = $('#<xsl:value-of select="@obj_provider"/>_table').DataTable( {
				        "destroy": true,
				        "responsive": <xsl:value-of select="@list_responsive"/>,
				        "processing": true,
				        "dom": '<xsl:value-of select="@distribution"/>',
				        ...
				        "lengthChange": <xsl:value-of select="@lengthchange"/>,
	        			"searching": <xsl:value-of select="@searching"/>,
			 		 	<xsl:for-each select="json">
							<xsl:for-each select="action">
					    ...
					    "language": {
						    ...
						    "infoEmpty":      "0 filas",
						    "infoFiltered":   "(Filtradas _MAX_ filas)",
						    "infoPostFix":    "",
  							"thousands":      ".",
						    "lengthMenu":     '#Lineas _MENU_',
						    ...
						    "aria": {
						        "sortAscending":  ": active orden ascendente",
						        "sortDescending": ": active orden descendente"
						    },
			                select: {
				                rows: {
				                    _: "Filas %d seleccionadas",
				                    0: "Click para seleccionar",
...
<xsl:template name="win_grid_table">
  	<xsl:variable name="fullname">dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@name"/></xsl:variable>
		<xsl:variable name="visibleicon"><xsl:value-of select="header/column[1]/@visible"/></xsl:variable>
		<xsl:variable name="halignmenticon"><xsl:value-of select="header/column[1]/@halignment"/></xsl:variable>
		<script>
			registerGrid('<xsl:value-of select="@tablename"/>',null,<xsl:value-of select="count(header/column)"/>,null,'<xsl:value-of select="@field"/>','<xsl:value-of select="$fullname"/>');
			 <xsl:if test="@modifiedonserver='true'">
				setChangeInputs(true);
			</xsl:if>					
		</script>
		<table class="win_list_table" border="0" cellpadding="0" cellspacing="0"  tabindex="0">
			<xsl:attribute name="id"><xsl:value-of select="@obj_provider"/>_table</xsl:attribute>
			<xsl:attribute name="class">table-grid<![CDATA[ ]]><xsl:value-of select="@class_table_responsive"/></xsl:attribute>
			...
			<tr>
				<xsl:for-each select="header">
					<xsl:for-each select="column">
						<th>
							<xsl:variable name="columnnamegrid">grid_<xsl:value-of select="../../@obj_provider"/>_<xsl:value-of select="position()"/></xsl:variable>
							<xsl:attribute name="id"><xsl:value-of select="$columnnamegrid"/></xsl:attribute>
	       					<xsl:attribute name="data-pos"><xsl:value-of select="position()"/></xsl:attribute>
							<xsl:attribute name="style">text-align: center;<xsl:if test="@visible='false'">display:none;</xsl:if></xsl:attribute>
	...
	<!--  						<script type="text/javascript" charset="ISO-8859-1">
								$("#<xsl:value-of select="$columnnamegrid"/>").resizable({
									resizeHeight : false,
									distance: 1
								});
							</script>-->
			...
			<xsl:for-each select="rows/row">
				...
				<tr tabindex="-1">
					...
					<xsl:for-each select="dblclick">
      				...
      				<xsl:if test="not(@deleted) and @rowpos mod 2 = 0"><xsl:attribute name="class">odd</xsl:attribute></xsl:if>
      				<xsl:if test="not(@deleted) and @rowpos mod 2 = 1"><xsl:attribute name="class">even</xsl:attribute></xsl:if>
      				<xsl:if test="@deleted"><xsl:attribute name="class">rowdeleted</xsl:attribute></xsl:if>
					<xsl:if test="@zobject"><xsl:attribute name="id"><xsl:value-of select="@zobject"/></xsl:attribute></xsl:if>
					<xsl:if test="@key"><xsl:attribute name="title"><xsl:value-of select="@key"/>;<xsl:value-of select="@descr"/></xsl:attribute></xsl:if>
					<!-- xsl:if test="../../@has_select='true'">
						<xsl:choose>
							<xsl:when test="@key">
								<xsl:attribute name="onclick">
					...
					<td>
						...
						<xsl:attribute name="style">text-align: center;<xsl:if test="$visibleicon='false'">display:none;</xsl:if></xsl:attribute>
						<xsl:if test="dblclick and icon">
								<xsl:call-template name="set_class" />
								<xsl:attribute name="align"><xsl:value-of select="$halignmenticon"/></xsl:attribute>
								<a >
									<xsl:if test="@tooltip">
										<xsl:attribute name="title"><xsl:value-of select="@tooltip"/></xsl:attribute>
									</xsl:if>								
			...
			<xsl:if test="@has_action_bar!='true'">
			   ...
			   <script>
				AAInit();
				AAFin('<xsl:value-of select="@obj_provider"/>_table',null,'<xsl:value-of select="@obj_provider"/>','<xsl:value-of select="rows/@zobject"/>',<xsl:value-of select="@is_multiple_select"/>,true,false,false,null);
			   </script>
			</xsl:if>			
	    	 <script>
				$("#<xsl:value-of select="@obj_provider"/>_table th").resizable({
					resizeHeight : false,
					distance: 1,
					start: function( event, ui ) {
						dropContentColumn(event,'<xsl:value-of select="@obj_provider"/>_table',ui);
			           
					},
					stop: function( event, ui ) {
						removeDropContentColumn(event,'<xsl:value-of select="@obj_provider"/>_table',ui);
					},
			         resize: function( event, ui ) {
						resizeDropContentColumn(event,'<xsl:value-of select="@obj_provider"/>_table',ui);
...
<!--  xsl:template name="win_grid_expand_table">
 	<xsl:variable name="fullname">dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@name"/></xsl:variable>
			...
			<xsl:for-each select="row">
					<div class="panel panel-default">
						<xsl:attribute name="id"><xsl:value-of select="@id"/></xsl:attribute>
					  	...
					  	<xsl:if test="not(@resize) and @class_responsive">
					  		<xsl:attribute name="class">ui-widget-content<xsl:text><![CDATA[ ]]></xsl:text>
					  		<xsl:value-of select="@class_responsive"/>
					  	</xsl:attribute></xsl:if>
						<xsl:apply-templates select="*"/>			
					</div>
					<script type="POSTSCRIPT">
        				$("#<xsl:value-of select="@id"/>").draggable({ scroll: true, containment: "parent" }).resizable({ scroll: true, containment: "parent" }); 
					</script>				
			</xsl:for-each>		
		</xsl:for-each>		
...
<xsl:template name="generate_basic_form">
	...
	<input type="hidden" name="dg_request">
	...
	<input type="hidden" name="src_sbmt">
		...
		<input type="hidden" name="dg_is_modal" /> <!--  comunica si la accion es modal -->
		<input type="hidden" name="dg_object_owner" />  <!--  comunica el object owner de la accion  -->
		<input type="hidden" name="dg_object_owner_dest" />  <!--  comunica el object owner dest de la accion swap  -->
		<input type="hidden" name="dg_object_owner_context" />  <!--  comunica el object owner context para lista extendidas -->
		...
		<input type="hidden" name="dg_multiple_owner_list_dest" />  <!--  comunica multiples owners de la accion para swap en refresh -->
		<input type="hidden" name="dg_is_multiple_owner" />  <!--  comunica si hay multiples owners de la accion  -->
		<input type="hidden" name="dg_row_select" />  <!--  comunica multiples owners de la accion  -->
		...
		<input type="hidden" name="dg_ajaxcontainer" /> <!--  informacion ajaxcontainer -->
...
<xsl:template name="set_class">
	<xsl:attribute name="class">
		<xsl:choose>
			<xsl:when test="@iseje='true'">zwlthcy matrix_eje <xsl:value-of select="@class_responsive"/></xsl:when>
			<xsl:when test="@marked='true'">zwltrmc matrix_marked <xsl:value-of select="@class_responsive"/></xsl:when>
			<xsl:when test="@word_wrap">x m <xsl:value-of select="@class_responsive"/></xsl:when>
			<xsl:when test="not(@word_wrap)">z m <xsl:value-of select="@class_responsive"/></xsl:when>
			<xsl:otherwise>zwltrc m <xsl:value-of select="@class_responsive"/></xsl:otherwise>
		</xsl:choose>
	</xsl:attribute>
</xsl:template>
...
<xsl:template name="generate_win_list_cell">
	...
	<xsl:param name="rowpos"></xsl:param>
...
<!-- 	<xsl:variable name="halignment"><xsl:value-of select="../../../header/column[ position() = $thispos ]/@halignment"/></xsl:variable> -->
		<xsl:variable name="objtype"><xsl:value-of select="@type" /></xsl:variable>
		<xsl:variable name="halignment"><xsl:value-of select="@halignment"/></xsl:variable>
	<td>
		...
		<!-- xsl:if test="@foreground or @background"-->
			<xsl:if test="@foreground or @background or (@height and @height!='20px' and @height!='20') or (@visibility and @visibility!='visible') or ($halignment and $halignment!='left') or (@valignment and @valignment!='')">
				<xsl:attribute name="style">
		...
		<xsl:choose>
								...
								<xsl:when test="actionlink">
										<a>
											...
											<xsl:if test="@foreground or @background">
												<xsl:attribute name="style">
													<xsl:if test="@foreground">color:<xsl:value-of select="@foreground"/>;</xsl:if>
													<xsl:if test="@background">background-color:<xsl:value-of select="@background"/>;</xsl:if>;null
												</xsl:attribute>
											</xsl:if>
							...
							<xsl:when test="@agrupado='true'">
								<div>
								    ...
								    <button>
								    	...
								    	<xsl:attribute name="id">btnGroupDrop<xsl:value-of select="$rowpos"/></xsl:attribute>
								        <xsl:if test="icon">
													<xsl:call-template name="render_icon">
														<xsl:with-param name="style_attr">
															<xsl:if test="@foreground">color:<xsl:value-of select="@foreground"/>;</xsl:if>
								        ...
								        <xsl:if test="not(icon)"><xsl:value-of select="@titulo"/></xsl:if>
								        <xsl:if test="with_caret='true'"><span class="caret"></span></xsl:if>
								    ...
								    <ul class="dropdown-menu pull-right">
										...
										<xsl:attribute name="aria-labelledby">btnGroupDrop<xsl:value-of select="$thispos"/></xsl:attribute>
			    ...
			    <xsl:when test="@selector">
					<input type="checkbox">
						<xsl:attribute name="onclick">
							selectSpecial(event,this,'<xsl:value-of select="@selector"/>')
						</xsl:attribute>
						<xsl:attribute name="onkeydown">
										...
										<xsl:when test="../.='S'">
											<xsl:choose>
												...
												<xsl:when test="not(../@image_true='') and not(contains(../@image_true,'.gif'))">
													<i>
														<xsl:attribute name='class'><xsl:value-of select="../@image_true"/></xsl:attribute>
													</i>
												</xsl:when>
												<xsl:otherwise>
										...
										<xsl:otherwise>
										    ...
										    <xsl:choose>
												...
												<xsl:when test="not(../@image_false='') and not(contains(../@image_false,'.gif'))">
													<i>
														<xsl:attribute name='class'><xsl:value-of select="../@image_false"/></xsl:attribute>
													</i>
												</xsl:when>
												<xsl:otherwise>
...
<xsl:template name="generate_win_grid_cell">
	...
	<xsl:variable name="objtype"><xsl:value-of select="../../../header/column[ position() = $thispos ]/@type" /></xsl:variable>
	<xsl:variable name="halignment"><xsl:value-of select="../../../header/column[ position() = $thispos ]/@halignment"/></xsl:variable>
	<xsl:variable name="visible"><xsl:value-of select="../../../header/column[ position() = $thispos ]/@visible"/></xsl:variable>
	<xsl:variable name="width"><xsl:value-of select="../../../header/column[ position() = $thispos ]/@width"/></xsl:variable>
	<td>
		<xsl:attribute name="name">name_<xsl:value-of select="position()"/></xsl:attribute>
		<xsl:if test="@axis"><xsl:attribute name="axis"><xsl:value-of select="@axis"/></xsl:attribute></xsl:if>
		<xsl:if test="@zobjectcell"><xsl:attribute name="id"><xsl:value-of select="@zobjectcell"/></xsl:attribute></xsl:if>
		...
		<!-- xsl:if test="@foreground or @background"-->
			<xsl:attribute name="style">
				<xsl:if test="@word_wrap">
					<xsl:call-template name="generate_inline_style_no_location">
						<xsl:with-param name="style_attr"><xsl:if test="@height">height:<xsl:value-of select="@height"/>;</xsl:if> white-space: pre; white-space: pre-wrap; white-space: pre-line; white-space: -pre-wrap; white-space: -o-pre-wrap; white-space: -moz-pre-wrap; white-space: -hp-pre-wrap; word-wrap: break-word; text-align:<xsl:value-of select="$halignment"/>;vertical-align:<xsl:value-of select="@valignment"/></xsl:with-param>
					</xsl:call-template>
				</xsl:if>
				<xsl:if test="not(@word_wrap)">
					<xsl:call-template name="generate_inline_style_no_location">
						<xsl:with-param name="style_attr"><xsl:if test="@height">height:<xsl:value-of select="@height"/>;</xsl:if> white-space:nowrap; text-align:<xsl:value-of select="$halignment"/>;vertical-align:<xsl:value-of select="@valignment"/>;</xsl:with-param>
					</xsl:call-template>
				</xsl:if>
...
<xsl:template match="color_field">
	...
	<div>
		...
		<xsl:attribute name="style">
			<xsl:call-template name="generate_inline_style">
				<xsl:with-param name="style_attr">border-top-width: 1px; border-right-width: 1px; border-left-width: 1px; border-style: solid; 
	...
	<xsl:variable name="fullname">dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@name"/></xsl:variable>
     <div>
          	<xsl:attribute name="class">
           	<xsl:choose>
				<xsl:when test="@visible and (@visible='true')">
					<xsl:if test="@size_responsive"><xsl:value-of select="@size_responsive"/></xsl:if><![CDATA[ ]]>form-group
					<xsl:if test="constraints/@required='true'"><![CDATA[ ]]>required</xsl:if>
				</xsl:when>
				<xsl:otherwise>form-group hidden</xsl:otherwise>
			</xsl:choose>
...
<xsl:template match="color_field_responsive">
	...
	<xsl:if test="@only-collapsed"><xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>nocollapsable</xsl:if>			
			...
			<div>
				...
				<xsl:attribute name="style">
					<xsl:call-template name="generate_inline_style_responsive">
						<xsl:with-param name="style_attr">border-width: 1px; border-style: solid; height:30px;
						border-color: #000000; 
						background-color: #<xsl:value-of select="text/."/>;
						</xsl:with-param>
					</xsl:call-template>
...
<xsl:template match="fromto_field_responsive">
	<xsl:variable name="fullname">dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@name"/></xsl:variable>
	<xsl:variable name="fullnamefrom">dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@propname_from"/></xsl:variable>
	<xsl:variable name="fullnameto">dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@propname_to"/></xsl:variable>
	     <div>
           	<xsl:attribute name="class">
      		   <xsl:choose>
					<xsl:when test="@visible and (@visible='true')"><xsl:if test="@size_responsive">
	...
	<xsl:if test="@only-collapsed"><xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>nocollapsable</xsl:if>		   
	         ...
	         <div>
		         ...
		         <div>
			        ...
			        <input>
			         	<xsl:attribute name="class">
			         		form-control<![CDATA[ ]]>input-sm
					   		<xsl:if test="@enteristab"><![CDATA[ ]]>enteristab</xsl:if>
					   		<![CDATA[ ]]>focuseable
				     	</xsl:attribute>
				     	<xsl:attribute name="id"><xsl:value-of select="$fullnamefrom"/></xsl:attribute>
						<xsl:call-template name="basic_generate_text_field_responsive">
							<xsl:with-param name="name"><xsl:value-of select="@propname_from"/></xsl:with-param>
						</xsl:call-template>
			         </input>
			         ...
			         <input>
				     	...
				     	<xsl:attribute name="id"><xsl:value-of select="$fullnameto"/></xsl:attribute>
						<xsl:call-template name="basic_generate_text_field_responsive">
							<xsl:with-param name="name"><xsl:value-of select="@propname_to"/></xsl:with-param>
						</xsl:call-template>
			         </input>
		         </div>
		         <script>
		         	initializeFromTo('<xsl:value-of select="$fullnamefrom"/>','<xsl:value-of select="$fullnameto"/>','<xsl:value-of select="text/."/>');
					registerNoConstraint('<xsl:value-of select="$fullname"/>',['<xsl:value-of select="$fullnamefrom"/>','<xsl:value-of select="$fullnameto"/>']);
					registerNoConstraint('<xsl:value-of select="$fullnamefrom"/>',['<xsl:value-of select="$fullnamefrom"/>']);
					registerNoConstraint('<xsl:value-of select="$fullnameto"/>',['<xsl:value-of select="$fullnameto"/>']);
				</script>
	         </div>
...
<xsl:template match="text_field_responsive">
				         ...
				         <div>
					         ...
					         <div>
						         ...
						         <input>
									...
									<span class="input-group-addon">
											<i>
												<xsl:attribute name="class"><xsl:value-of select="@pop_up_icon"/></xsl:attribute>
					...
					<xsl:otherwise>
			  			<xsl:if test="@size_responsive and @size_responsive='inline_component'">
						   <input>
				         	<xsl:attribute name="class">
				         		form-control<![CDATA[ ]]>input-sm
						   		<xsl:if test="@outstanding">><![CDATA[ ]]>outstanding</xsl:if>
					   			<xsl:if test="@enteristab"><![CDATA[ ]]>enteristab</xsl:if>
					   			<![CDATA[ ]]>focuseable
				         	</xsl:attribute>
				         	<xsl:attribute name="formnovalidate"></xsl:attribute>
	...
	<script>
		<xsl:if test="@connect_control">
			...
			'<xsl:value-of select="@connect_control"/>',
			function() {
				hideSelectMetaItemsEdit('<xsl:value-of select="$fullname"/>','<xsl:value-of select="@connect_control_field"/>','dgf_form_<xsl:value-of select="@connect_control"/>_fd-swap_zone','<xsl:value-of select="@connect_control_operator"/>','<xsl:value-of select="@connect_control_datatype"/>');
				hideSelectMetaItemsEdit('<xsl:value-of select="$fullname"/>','<xsl:value-of select="@connect_control_field"/>','dgf_form_<xsl:value-of select="@connect_control"/>_fd-swap_zone_to','<xsl:value-of select="@connect_control_operator"/>','<xsl:value-of select="@connect_control_datatype"/>');
			});
		</xsl:if>
	</script>
</xsl:template>
...
<xsl:template match="text_label_responsive">
	...
	<xsl:if test="@only-collapsed"><xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>nocollapsable</xsl:if>				
				...
				<xsl:if	test="not(@size_responsive) or @size_responsive!='inline_component'">
					...
					<div>
						<xsl:attribute name="class">data_label<![CDATA[ ]]>
							<xsl:if test="@field_class"><xsl:value-of select="@field_class" /></xsl:if>
						</xsl:attribute>
						<div>
							<xsl:attribute name="class">
								input-group<![CDATA[ ]]>sinborde<![CDATA[ ]]>
							</xsl:attribute>
							<input>
								<xsl:attribute name="class">
									form-control<![CDATA[ ]]>sinborde<![CDATA[ ]]>input-sm
						   			
						   			<xsl:if test="@outstanding"> outstanding</xsl:if>
					         	</xsl:attribute>
								<xsl:attribute name="formnovalidate"></xsl:attribute>
								<xsl:attribute name="type">text</xsl:attribute>
								<xsl:attribute name="readonly">readonly</xsl:attribute>
	...
	<xsl:choose>
			<xsl:when test="@diferido='true'">
				<div>
					<xsl:call-template name="basic_generate_component_responsive"/>
					<div>
						<xsl:attribute name="id"><xsl:value-of select="@htmlarea_content"/></xsl:attribute>
						<xsl:attribute name="style"><xsl:call-template name="generate_inline_style_responsive"/></xsl:attribute>
					</div>
				</div>
				<script type="POSTSCRIPT">
...
<xsl:template match="collapsable_responsive"> 
	<xsl:variable name="fullname"><xsl:value-of select="@name"/></xsl:variable>
 	<button type="button">
       	<xsl:attribute name="class"><xsl:value-of select="@class_responsive"/></xsl:attribute>
...
<xsl:template match="check_box_responsive_noform"> 
    ...
    <label>
		...
		<INPUT type="checkbox">
			<xsl:attribute name="name"><xsl:value-of select="$fullname"/></xsl:attribute>
			<xsl:attribute name="id"><xsl:value-of select="$fullname"/></xsl:attribute>
			<xsl:if test="@mode"><xsl:attribute name="data-toggle"><xsl:value-of select="@mode"/></xsl:attribute></xsl:if>
			<xsl:if test="@dataon"><xsl:attribute name="data-on"><xsl:value-of select="@dataon"/></xsl:attribute></xsl:if>
			<xsl:if test="@dataoff"><xsl:attribute name="data-off"><xsl:value-of select="@dataoff"/></xsl:attribute></xsl:if>
			<xsl:if test="@datasize"><xsl:attribute name="data-size"><xsl:value-of select="@datasize"/></xsl:attribute></xsl:if>
			<xsl:if test="@datawidth"><xsl:attribute name="data-width"><xsl:value-of select="@datawidth"/></xsl:attribute></xsl:if>
			<xsl:if test="@dataheight"><xsl:attribute name="data-height"><xsl:value-of select="@dataheight"/></xsl:attribute></xsl:if>
			<xsl:if test="@dataonstyle"><xsl:attribute name="data-onstyle"><xsl:value-of select="@dataonstyle"/></xsl:attribute></xsl:if>
			<xsl:if test="@dataoffstyle"><xsl:attribute name="data-offstyle"><xsl:value-of select="@dataoffstyle"/></xsl:attribute></xsl:if>
			<xsl:if test="@datastyle"><xsl:attribute name="data-style"><xsl:value-of select="@datastyle"/></xsl:attribute></xsl:if>
			<xsl:if test="@checked='S'"><xsl:attribute name="checked"></xsl:attribute></xsl:if>
			<xsl:if test="@editable!='true'">
				<xsl:attribute name="readonly">readonly</xsl:attribute>
				<xsl:attribute name="tabindex">-1</xsl:attribute>
			...
			<xsl:attribute name="class">
						<xsl:if test="@mode='square'">
	 							regular-checkbox
						</xsl:if>
						<xsl:if test="@mode='bigsquare'">
	 							regular-checkbox<![CDATA[ ]]>big-checkbox
						</xsl:if>
						<xsl:if test="@mode='cicle'">
	 							regular-radio
						</xsl:if>
						<xsl:if test="@mode='bigcircle'">
	 							regular-radio<![CDATA[ ]]>big-radio
						</xsl:if>
						<![CDATA[ ]]>pss-checkbox
			</xsl:attribute>
			<xsl:for-each select="onblur">
		...
		<script type="text/javascript">
           	 inicializeCheckBox('<xsl:value-of select="$fullname"/>');
        </script>
...
<xsl:template match="check_box_responsive"> 
	...
	<xsl:variable name="onchange_event_var">
				<xsl:if test="@mode!='toggle'">
					document.getElementById('<xsl:value-of select="$fullname"/>').value=this.checked?'S':'N';
				</xsl:if>
	</xsl:variable>
	...
	<xsl:if test="@only-collapsed"><xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>nocollapsable</xsl:if>				
				...
				<xsl:if test="@align='TOP'">
	    	        <label>
			         	<xsl:attribute name="for"><xsl:value-of select="$fullname"/><xsl:if test="@mode!='toggle'">_check</xsl:if></xsl:attribute>
			 			<xsl:if test="@label_class"><xsl:attribute name="class"><xsl:value-of select="@label_class"/></xsl:attribute></xsl:if>
			         	<xsl:value-of select="@label_lateral"/>
			         </label>
		         </xsl:if>
		       	<div>
					...
					<xsl:call-template name="basic_generate_component_responsive" />
						<xsl:if test="@mode!='toggle'">
	 						<xsl:attribute name="class">
		 						input-group 
  							s<xsl:if test="@field_class"><xsl:value-of select="@field_class"/></xsl:if>
	 						</xsl:attribute>
						</xsl:if>
					<INPUT type="checkbox">
						<xsl:attribute name="name"><xsl:value-of select="$fullname"/><xsl:if test="@mode!='toggle'">_check</xsl:if></xsl:attribute>
						<xsl:attribute name="id"><xsl:value-of select="$fullname"/><xsl:if test="@mode!='toggle'">_check</xsl:if></xsl:attribute>
						<xsl:if test="@mode and @mode='toggle'"><xsl:attribute name="data-toggle"><xsl:value-of select="@mode"/></xsl:attribute></xsl:if>
						<xsl:if test="@dataon"><xsl:attribute name="data-on"><xsl:value-of select="@dataon"/></xsl:attribute></xsl:if>
						<xsl:if test="@dataoff"><xsl:attribute name="data-off"><xsl:value-of select="@dataoff"/></xsl:attribute></xsl:if>
						<xsl:if test="@datasize"><xsl:attribute name="data-size"><xsl:value-of select="@datasize"/></xsl:attribute></xsl:if>
						<xsl:if test="@datawidth"><xsl:attribute name="data-width"><xsl:value-of select="@datawidth"/></xsl:attribute></xsl:if>
						...
						<xsl:if test="@checked='S'"><xsl:attribute name="checked"></xsl:attribute></xsl:if>
  						<xsl:attribute name="class">
							<xsl:if test="@mode='single'">
		 							form-control<![CDATA[ ]]>input-sm<![CDATA[ ]]>
							</xsl:if>
							<xsl:if test="@mode='square'">
		 							regular-checkbox
							</xsl:if>
						...
						<xsl:if test="@enteristab">
							...
							<![CDATA[ ]]>pss-checkbox
							<![CDATA[ ]]>focuseable
							<xsl:if test="@refreshForm='true'"><![CDATA[ ]]>refreshform</xsl:if>
						</xsl:attribute>
						<xsl:if test="@editable!='true'">
							<xsl:attribute name="readonly">readonly</xsl:attribute>
							<xsl:attribute name="tabindex">-1</xsl:attribute>
					...
					<xsl:if test="@mode!='toggle'">
	 						<span>
		 						<xsl:attribute name="for"><xsl:value-of select="$fullname"/>_check</xsl:attribute>
			...
			<xsl:if test="@connect_control">
				<script>
					...
					'<xsl:value-of select="@connect_control"/>',
					function() {
						hideSelectMetaItemsCheck('<xsl:value-of select="$fullname"/>','<xsl:value-of select="@connect_control_field"/>','dgf_form_<xsl:value-of select="@connect_control"/>_fd-swap_zone','<xsl:value-of select="@connect_control_operator"/>','<xsl:value-of select="@connect_control_datatype"/>');
						hideSelectMetaItemsCheck('<xsl:value-of select="$fullname"/>','<xsl:value-of select="@connect_control_field"/>','dgf_form_<xsl:value-of select="@connect_control"/>_fd-swap_zone_to','<xsl:value-of select="@connect_control_operator"/>','<xsl:value-of select="@connect_control_datatype"/>');
					});
			    </script>
			</xsl:if>
				<xsl:call-template name="basic_generate_form_component_scripts_out"/>
	...
	<xsl:if test="@only-collapsed"><xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>nocollapsable</xsl:if>				
            	...
            	<xsl:choose>
	    		         ...
	    		         <a>
							...
							<xsl:if test="action">
								<xsl:if test="@refreshForm='true'">
									<xsl:for-each select="action">
										<xsl:attribute name="onClick">
											goToRefreshForm('do-PartialRefreshForm', '<xsl:value-of select="$fullname"/>', '<xsl:value-of select="../action/@obj_provider"/>','<xsl:value-of select="../action/@object_owner_id"/>', getLastModal(), '<xsl:value-of select="../action/@id_action"/>');
											 var evt = event ? event:window.event;
				 							 if (evt.stopPropagation)    evt.stopPropagation();
				 							 if (evt.cancelBubble!=null) evt.cancelBubble = true;
											 evt.preventDefault();
		            ...
		            <a> 
		            	<xsl:call-template name="basic_generate_component_responsive">
								<xsl:with-param name="specialClass">
									input-group btn<![CDATA[ ]]><xsl:value-of select="@class_responsive"/>
								 	<xsl:if test="@enteristab"><![CDATA[ ]]>enteristab</xsl:if>
								 	<![CDATA[ ]]>focuseable
								</xsl:with-param>
						</xsl:call-template>
						...
						<xsl:if test="action">
							<xsl:if test="@refreshForm='true'">
								<xsl:for-each select="action">
									<xsl:attribute name="onClick">
										goToRefreshForm('do-PartialRefreshForm',
											<xsl:choose>
										 ...
										 , '<xsl:value-of select="../action/@obj_provider"/>','<xsl:value-of select="../action/@object_owner_id"/>', getLastModal(), '<xsl:value-of select="../action/@id_action"/>');
										 var evt = event ? event:window.event;
			 							 if (evt.stopPropagation)    evt.stopPropagation();
			 							 if (evt.cancelBubble!=null) evt.cancelBubble = true;
										 evt.preventDefault();
						...
						<xsl:if test="icon and not(@align='right')">
							<xsl:call-template name="render_icon">
								<xsl:with-param name="style_attr"><xsl:if test="icon/@style_image=''">padding-right:10px;</xsl:if></xsl:with-param>
							</xsl:call-template>
						</xsl:if>
				        <xsl:if test="@label">
								<xsl:value-of select="@label"/>
						...
						<xsl:if test="icon and @align='right'">
							<xsl:call-template name="render_icon">
								<xsl:with-param name="style_attr"><xsl:if test="icon/@style_image=''">padding-left:10px;</xsl:if></xsl:with-param>
							</xsl:call-template>
						</xsl:if>
					</a>
            		</xsl:otherwise>
...
<xsl:template match="web_button">
    <a class="web_button"  >  
		...
		<xsl:if test="@refreshForm='true'">
			<xsl:for-each select="action">
				<xsl:attribute name="onClick">
					goToRefreshForm('do-PartialRefreshForm', '<xsl:value-of select="@label"/>', '<xsl:value-of select="../action/@obj_provider"/>','<xsl:value-of select="../action/@object_owner_id"/>', getLastModal(), '<xsl:value-of select="../action/@id_action"/>');
				</xsl:attribute>
			</xsl:for-each>
		</xsl:if>	
		<xsl:if test="not(@refreshForm='true')">
		...
		<tr><td>
				...
				<xsl:if test="@isSearcheable='true' and @editable!='false'">
					<a>
						...
						<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_combo_button</xsl:attribute>
						<xsl:attribute name="onclick">
							onFocusObjectById('<xsl:value-of select="$fullname"/>');
							formLovChange('<xsl:value-of select="$fullname"/>','<xsl:value-of select="item/@objectOwner"/>');</xsl:attribute>
						<img>
							<xsl:attribute name="src"><xsl:value-of select="$skin_path"/>/images/comboButton.gif</xsl:attribute>
						</img>
			...
			<xsl:if test="@editable!='false'">
				<td width="23">
					    <a class="form_lov_button_open"   >  
							...
							<xsl:attribute name="onclick"> 
								onFocusObjectById('<xsl:value-of select="$fullname"/>');
							   	winLovOpen(this,'<xsl:value-of select="$fullname"/>',
					               '<xsl:value-of select="item/@objectOwner"/>',
					               '<xsl:value-of select="item/@objectAction"/>',
							       '<xsl:value-of select="item/@obj_provider"/>',
							        <xsl:choose><xsl:when test="item/@refreshFormOnSelect='true'">true</xsl:when><xsl:otherwise>false</xsl:otherwise></xsl:choose>,
							        false,'<xsl:value-of select="item/@multiple"/>',false);
							</xsl:attribute>
			        		<span  >
							</span>
						</a>
...
<xsl:template match="swaplist_responsive">
	<xsl:variable name="fullname">dgf_<xsl:value-of select="@form_name"/>_fd-swap</xsl:variable>
	    ...
	    <div class="col-xs-5">
		    <input>
		    	<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_search_a</xsl:attribute>
									
		    	<xsl:attribute name="onkeyup">runUpKey('<xsl:value-of select="@form_name"/>');</xsl:attribute>
		    </input>
		    <div class="pull-right">
		    	<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_label_hidden_a</xsl:attribute>
			</div>
		</div>
	    <div class="col-xs-2">
		</div>
	    <div class="col-xs-5">
		    <input>
		    	<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_search_b</xsl:attribute>
		    	<xsl:attribute name="onkeyup">runUpKey('<xsl:value-of select="@form_name"/>');</xsl:attribute>
		    </input>
		    <div  class="pull-right">
		    	<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_label_hidden_b</xsl:attribute>
			</div>
		</div>
	    <div class="col-xs-5">
	        <select name="from[]" class="form-control" size="13" multiple="multiple">
	        	<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_zone</xsl:attribute>
					...
					<xsl:for-each select="items">
			            <option>
							...
							<xsl:for-each select="extradata">
								<xsl:attribute name="data-{@extradata}"><xsl:value-of select="@extradata_value"/></xsl:attribute>
							</xsl:for-each>							
							<xsl:value-of select="@description"/>
			            </option>
					</xsl:for-each>
	    ...
	    <div class="col-xs-2">
	        <button type="button" class="btn btn-primary2 btn-block"><xsl:attribute name="id"><xsl:value-of select="$fullname"/>_zone_undo</xsl:attribute>undo</button>
	        <button type="button" class="btn btn-default btn-block"><xsl:attribute name="id"><xsl:value-of select="$fullname"/>_zone_rightAll</xsl:attribute><i class="glyphicon glyphicon-forward"></i></button>
	        <button type="button" class="btn btn-default btn-block"><xsl:attribute name="id"><xsl:value-of select="$fullname"/>_zone_rightSelected</xsl:attribute><i class="glyphicon glyphicon-chevron-right"></i></button>
	        <button type="button" class="btn btn-default btn-block"><xsl:attribute name="id"><xsl:value-of select="$fullname"/>_zone_leftSelected</xsl:attribute><i class="glyphicon glyphicon-chevron-left"></i></button>
	        <button type="button" class="btn btn-default btn-block"><xsl:attribute name="id"><xsl:value-of select="$fullname"/>_zone_leftAll</xsl:attribute><i class="glyphicon glyphicon-backward"></i></button>
	        <button type="button" class="btn btn-warning btn-block"><xsl:attribute name="id"><xsl:value-of select="$fullname"/>_zone_redo</xsl:attribute>redo</button>
	    </div>
	    
	    <div class="col-xs-5">
	        <select name="to[]" class="form-control" size="13" multiple="multiple">
	        	<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_zone_to</xsl:attribute>
	        	<xsl:attribute name="style">overflow-x:auto</xsl:attribute>
	    ...
	    <script type="text/javascript">
		   <xsl:variable name="v_provider"><xsl:value-of select="@obj_provider"/></xsl:variable>
			inicializeSwaplist('<xsl:value-of select="$fullname"/>_zone');
			AAInit();
			<xsl:for-each select="actions/action">
				<xsl:if test="allowed_win">
					AAAdd('<xsl:value-of select="@id"/>',<xsl:value-of select="@win"/>,[<xsl:for-each select="allowed_win">'<xsl:value-of select="@id"/>',</xsl:for-each>null],<xsl:value-of select="@multiple"/>);
			...
			AAFin('<xsl:value-of select="$fullname"/>_zone_to','<xsl:value-of select="//win_action_bar[@provider=$v_provider]/@name"/>','<xsl:value-of select="@obj_provider"/>','<xsl:value-of select="@zobject"/>',true,true,'0',false,'<xsl:value-of select="@zobjectdest"/>');
			swapStartFilters('<xsl:value-of select="@form_name"/>','<xsl:value-of select="$fullname"/>');
			registerNoConstraint('<xsl:value-of select="$fullname"/>',['<xsl:value-of select="$fullname"/>_zone_to']);
		</script>
	</div>
</xsl:template>
...
<xsl:template match="options_responsive">
	...
	<xsl:for-each select="option">
        <input type="button"> 
           	<xsl:attribute name="class">
					btn<xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text><xsl:value-of select="@class_responsive"/><xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text><xsl:value-of select="@size_button"/>
			</xsl:attribute>
...
<xsl:template match="win_lov_responsive">
	...
	<xsl:if test="@only-collapsed"><xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>nocollapsable</xsl:if>				
		        ...
		        <div>
			        ...
			        <div>
						<xsl:attribute name="class">input-group
		 		   		</xsl:attribute>
						<xsl:if test="@maxwidth or @maxheight">
							<xsl:attribute name="style">
					         	<xsl:if test="@maxwidth">max-width:<xsl:value-of select="@maxwidth"></xsl:value-of>px;</xsl:if>
					         	<xsl:if test="@maxheight">max-height:<xsl:value-of select="@maxheight"></xsl:value-of>px;</xsl:if>
					        </xsl:attribute>
						</xsl:if>
					    ...
					    <select>
							...
							<xsl:if test="@connect_control">
								<xsl:attribute name="onChange">
									...
									runUpKey('<xsl:value-of select="@connect_control"/>');
									return false;
								</xsl:attribute>
							</xsl:if>
								...
								<span>
									...
									<button>
										...
										<xsl:attribute name="onclick">
											onFocusObjectById('<xsl:value-of select="$fullname"/>');
										   	winLovOpen(this,'<xsl:value-of select="$fullname"/>',
								               '<xsl:value-of select="@objectOwner"/>',
								               '<xsl:value-of select="@objectAction"/>',
										       '<xsl:value-of select="@obj_provider"/>',
										        <xsl:choose><xsl:when test="@refreshFormOnSelect='true'">true</xsl:when><xsl:otherwise>false</xsl:otherwise></xsl:choose>,
										        <xsl:value-of select="@multiple"/>,true);
										</xsl:attribute>
							
										<span><xsl:attribute name="class">fa fa-search</xsl:attribute></span>
									</button>
								</span>
							</xsl:if>
							<xsl:if test="@show_lupa='true'">
								<span>
									...
									<button>
										...
										<xsl:attribute name="type">button</xsl:attribute>
										<xsl:attribute name="onclick">
											clearSelect2('<xsl:value-of select="$fullname"/>',  <xsl:choose><xsl:when test="@refreshFormOnSelect='true'">true</xsl:when><xsl:otherwise>false</xsl:otherwise></xsl:choose>);
										</xsl:attribute>
										...
										<span>
											<xsl:attribute name="class">input-group-btn</xsl:attribute>
									            <button > 
													<xsl:attribute name="type">button</xsl:attribute>
									            	<xsl:call-template name="basic_generate_component_responsive">
															<xsl:with-param name="specialClass">btn <xsl:value-of select="@class_responsive"/></xsl:with-param>
															<xsl:with-param name="style_attr"><xsl:if test="not(icon)">padding-bottom: 2px;padding-top: 2px;</xsl:if></xsl:with-param>
													</xsl:call-template>
													<xsl:if test="@tooltip">
														<xsl:attribute name="title"><xsl:value-of select="@tooltip"/></xsl:attribute>
													</xsl:if>
...
1													</xsl:if>
				...
				<script type="text/javascript" charset="ISO-8859-1">
					   inicializeSelect2('<xsl:value-of select="$fullname"/>','<xsl:value-of select="@placeholder"/>',<xsl:value-of select="@MaximumSelectionLength"/>,<xsl:value-of select="@MinimumInputLength"/>,<xsl:value-of select="@show_key"/>,<xsl:value-of select="@multiple"/>,<xsl:value-of select="show_lupa"/>);
				...
				<script charset="ISO-8859-1">
					...
					<xsl:if test="@intable='true'">
						rdg('<xsl:value-of select="@table_name"/>','<xsl:value-of select="@name"/>',['<xsl:value-of select="$fullname"/>'],<xsl:value-of select="@table_row"/>,<xsl:value-of select="@table_column"/>);	
					</xsl:if>	
					<xsl:if test="not(@intable='true')">
						registerNoConstraint('<xsl:value-of select="$fullname"/>',['<xsl:value-of select="$fullname"/>']<xsl:if test="constraints/@required='true'">,'<xsl:value-of select="constraints/@description"/>',true</xsl:if>);
					</xsl:if>
					<xsl:if test="@isSearcheable='true' and @editable!='false'">
						registerResponsiveFormLov('<xsl:value-of select="$fullname"/>',<xsl:value-of select="@show_key"/>,<xsl:value-of select="@refreshFormOnSelect"/>, '<xsl:value-of select="@objectOwner"/>','<xsl:value-of select="@objectAction"/>','<xsl:value-of select="@multiple"/>','<xsl:value-of select="@obj_provider"/>',<xsl:if test="@detect_changes='true'">true</xsl:if><xsl:if test="not(@detect_changes='true')">false</xsl:if>);
					</xsl:if>
				</script>
		     	<script>
					<xsl:if test="@connect_control">
	...
	<xsl:param name="specialClass" select="'null'" />
			...
			<xsl:variable name="fullname">dgf_<xsl:value-of select="@form_name"/>_fd-<xsl:value-of select="@name"/></xsl:variable>
		     ...
		     <div>
				...
				<script charset="ISO-8859-1">
					registerNoConstraint('<xsl:value-of select="$fullname"/>',['<xsl:value-of select="$fullname"/>']<xsl:if test="constraints/@required='true'">,true</xsl:if>);
				</script>
				<ul>
					<xsl:attribute name="class">
 						form-control input-sm
...
<xsl:template match="multiple_check_responsive">
	...
	<xsl:if test="@only-collapsed"><xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>nocollapsable</xsl:if>		
		...
		<div>
			<xsl:attribute name="style">
				<xsl:call-template name="generate_inline_style_responsive"/>;overflow:auto;
			</xsl:attribute>
			<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_div_main</xsl:attribute>
			<xsl:attribute name="name"><xsl:value-of select="$fullname"/>_div_main</xsl:attribute>
...
<xsl:template name="do_multiple_check_options">
	...
	<xsl:for-each select="item">
		...
		<xsl:if test="../@editable='false'">
			<xsl:if test="../@onlyshowselected='false' or @selected">
				<input type="checkbox" disabled="disabled"> 
					<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_option_<xsl:value-of select="@id_name"/>_check</xsl:attribute>
					<xsl:attribute name="name"><xsl:value-of select="$fullname"/>_option_<xsl:value-of select="@id_name"/>_check</xsl:attribute>
					...
					<xsl:attribute name="data-on"><xsl:value-of select="@description"/></xsl:attribute>
					<xsl:attribute name="data-off"><xsl:value-of select="@description"/></xsl:attribute>
					<xsl:attribute name="checked"></xsl:attribute>
					<xsl:attribute name="value"><xsl:value-of select="@id"/></xsl:attribute>
					
				</input>
				<script type="text/javascript">
		    	    inicializeCheckBox('<xsl:value-of select="$fullname"/>_option_<xsl:value-of select="@id_name"/>_check');
		            <xsl:if test="@selected">
		            	fillMarked('<xsl:value-of select="$fullname"/>_option_<xsl:value-of select="@id_name"/>_check','<xsl:value-of select="$fullname"/>','<xsl:value-of select="@id"/>');
		            </xsl:if>
		        </script>
			</xsl:if>
		</xsl:if>
		<xsl:if test="../@editable!='false'">
		    	 ...
		    	 <label>
		         	<xsl:attribute name="for"><xsl:value-of select="$fullname"/>_option_<xsl:value-of select="@id_name"/>_check</xsl:attribute>
		 			<xsl:if test="@label_class"><xsl:attribute name="class"><xsl:value-of select="@label_class"/></xsl:attribute></xsl:if>
		         	<xsl:value-of select="@label_lateral"/>
					<INPUT type="checkbox">
						<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_option_<xsl:value-of select="@id_name"/>_check</xsl:attribute>
						...
						<xsl:attribute name="onclick">javascript:
							...
							fillMark('<xsl:value-of select="$fullname"/>_option_<xsl:value-of select="@id_name"/>_check','<xsl:value-of select="$fullname"/>','<xsl:value-of select="@id"/>');
							<xsl:if test="../@refreshForm='true'">
								goToRefreshForm('do-PartialRefreshForm', '<xsl:value-of select="$fullname"/>', '<xsl:value-of select="../action/@obj_provider"/>','<xsl:value-of select="../action/@object_owner_id"/>', getLastModal(), '<xsl:value-of select="../action/@id_action"/>');
							</xsl:if>
							<xsl:if test="@connect_control">
...
<xsl:template match="multiple_list_responsive">
    ...
    <div>
		...
		<ul>
			<xsl:attribute name="class">gallery<![CDATA[ ]]><xsl:value-of select="@class_responsive"></xsl:value-of></xsl:attribute>
			<xsl:attribute name="style">
				<xsl:call-template name="generate_inline_style_responsive"/>;overflow:auto;
			</xsl:attribute>
			<xsl:attribute name="id"><xsl:value-of select="$fullname"/>_div_main</xsl:attribute>
		...
		<script charset="ISO-8859-1">
         ...
         <xsl:if test="@connect_control">
			<script>
				...
				'<xsl:value-of select="@connect_control"/>',
				function() {
					hideSelectMetaItemsRadio('<xsl:value-of select="$fullname"/>','<xsl:value-of select="@connect_control_field"/>','dgf_form_<xsl:value-of select="@connect_control"/>_fd-swap_zone','<xsl:value-of select="@connect_control_operator"/>','<xsl:value-of select="@connect_control_datatype"/>');
					hideSelectMetaItemsRadio('<xsl:value-of select="$fullname"/>','<xsl:value-of select="@connect_control_field"/>','dgf_form_<xsl:value-of select="@connect_control"/>_fd-swap_zone_to','<xsl:value-of select="@connect_control_operator"/>','<xsl:value-of select="@connect_control_datatype"/>');
				});
		    </script>
		</xsl:if>
	...
	<xsl:for-each select="item">
		<xsl:if test="../@editable='false'">
			<xsl:if test="../@onlyshowselected='false' or @selected">
				<li>
					...
					<xsl:attribute name="class">
						multiple_list_option_readonly
				 	 	<xsl:if test="@selected and @selected='true'">
							selected
				 	 	</xsl:if>
					</xsl:attribute>
					<div>
		...
		<xsl:if test="../@editable!='false'">
			...
			<li>
				...
				<xsl:attribute name="name"><xsl:value-of select="$fullname"/>_option_<xsl:value-of select="@id"/></xsl:attribute>
					<xsl:attribute name="onclick">javascript:
					set_class('<xsl:value-of select="$fullname"/>_div_main','li','multiple_list_option');
					document.getElementById('<xsl:value-of select="$fullname"/>').value='<xsl:value-of select="@id"/>';
					document.getElementById('<xsl:value-of select="$fullname"/>_option_<xsl:value-of select="@id"/>').className ='multiple_list_option_select';
					<xsl:if test="@detect_changes='true'">setChangeInputs(true);</xsl:if>
					<xsl:if test="@onCalculate and @isCalculeOthersFields">
...
<xsl:template name="do_treeview_div">
	...
	<script type="text/javascript" charset="ISO-8859-1">
		...
		var k = <xsl:value-of select="$varname"/>_d;
		var p = { fullname: '<xsl:value-of select="$fullname"/>',
		          provider:'<xsl:value-of select="action/@obj_provider"/>',
		          owner:'<xsl:value-of select="action/@object_owner_id"/>',
		          action:'<xsl:value-of select="action/@id_action"/>'};
		var i=saveMemoryArray(p);
 		var s='uf('+i+',';
		var g='gr('+i+')';
		var m= [
			<xsl:for-each select="item">
			{
			 ...
			 i:'<xsl:value-of select="@id_tree"/>',
			 p:'<xsl:value-of select="@parent"/>',
			 d:'<xsl:value-of select="@description"/>',
			 f:'<xsl:value-of select="$fullname"/>',
			 <xsl:if test="@icon=@icon_open">
				 c:'<xsl:value-of select="@icon"/>',
				 o:null
			 </xsl:if>
			 <xsl:if test="@icon!=@icon_open">
				 c:'<xsl:value-of select="@icon"/>',
				 o:'<xsl:value-of select="@icon_open"/>'
			 </xsl:if>
			 ,x:<xsl:if test="not(../@editable='true' and @elegible='true')">
				null
			  </xsl:if>
		...
		bt(k,m,'<xsl:value-of select="$varname"/>_tree','<xsl:value-of select="@value"/>','<xsl:value-of select="@title"/>','<xsl:value-of select="@icon"/>');
	</script>
</xsl:template>
...
<xsl:template match="file_responsive">
	...
	<xsl:if test="@only-collapsed"><xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>nocollapsable</xsl:if>				
		 		...
		 		<xsl:if test="@editable!='false'">
					<iframe src="html/upload.html"  frameborder="0"  seamless="seamless">
		     ...
		     <div>
            	...
            	<xsl:attribute name="class">
					...
					<xsl:if test="@mode='GOOGLEPAY'">
						...
						<script type="POSTSCRIPT">
							createPaymentManager('<xsl:value-of select="$fullname"/>',{
	    					    countryCode: '<xsl:value-of select="@country"/>',
							    currencyCode: '<xsl:value-of select="@currency"/>',
							    totalPriceStatus: '<xsl:value-of select="@status"/>',
							    totalPrice: '<xsl:value-of select="@price"/>'
							  },
								function() {goToRefreshForm('do-PartialRefreshForm', '<xsl:value-of select="$fullname"/>', '<xsl:value-of select="action/@obj_provider"/>','<xsl:value-of select="action/@object_owner_id"/>', getLastModal(), '<xsl:value-of select="action/@id_action"/>', null<xsl:if test="action/@object_context_id">,'<xsl:value-of select="action/@object_context_id"/>'</xsl:if>);}
							  );
					...
					<xsl:if test="@mode='MERCADOPAGO'">
						<div class="col-sm-offset-3 col-sm-6">
						 	<fieldset id="pay-panel" class="panel-default shadow "  style="height:250px"><legend>Formulario de pago</legend>
							  <div class="panel-body" id="form-checkout">
								  <div class="col-sm-12"><div class="input-group"><input type="text" class="form-control input-sm" name="cardNumber" id="form-checkout__cardNumber" /></div></div>
								  <div class="col-sm-3"><div class="input-group"><input type="text" class="form-control input-sm" name="cardExpirationMonth" id="form-checkout__cardExpirationMonth" /></div></div>
								  <div class="col-sm-3"><div class="input-group"><input type="text" class="form-control input-sm" name="cardExpirationYear" id="form-checkout__cardExpirationYear" /></div></div>
								  <div class="col-sm-6"><div class="input-group"><input type="text" class="form-control input-sm" name="cardholderName" id="form-checkout__cardholderName"/></div></div>
								  <div class="col-sm-3"><div class="input-group"><input type="text" class="form-control input-sm" name="securityCode" id="form-checkout__securityCode" /></div></div>
								  <div class="col-sm-3"><div class="input-group"><select name="issuer"  class="form-control input-sm" id="form-checkout__issuer"></select></div></div>
								  <div class="col-sm-4"><div class="input-group"><select name="identificationType" class="form-control input-sm" id="form-checkout__identificationType"></select></div></div>
								  <div class="col-sm-6"><div class="input-group"><input type="text" class="form-control input-sm" name="identificationNumber" id="form-checkout__identificationNumber"/></div></div>
								  <div class="col-sm-6"><div class="input-group"><select name="installments" class="form-control input-sm" id="form-checkout__installments"></select></div></div>
								  <div class="col-sm-3"><div class="input-group"><button type="submit" class="btn btn-primary" id="form-checkout__submit">Pagar</button></div></div>
						...
						<script type="POSTSCRIPT">
							createPaymentMercadopagoManager('<xsl:value-of select="$fullname"/>',{
	    					    countryCode: '<xsl:value-of select="@country"/>',
							    currencyCode: '<xsl:value-of select="@currency"/>',
							    totalPriceStatus: '<xsl:value-of select="@status"/>',
							    totalPrice: '<xsl:value-of select="@price"/>'
...
<xsl:template match="radio_button_set_responsive">
	...
	<xsl:choose>
		<xsl:when test="$orientation='toogle'">
		    <DIV>
		       	<xsl:attribute name="class">
		    	  	<xsl:if test="@size_responsive"><xsl:value-of select="@size_responsive"/><xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text></xsl:if>
		           	<xsl:choose>
						<xsl:when test="@visible and (@visible='true')"> form-group
							<xsl:if test="constraints/@required='true'"><![CDATA[ ]]>required</xsl:if>
					...
					<xsl:for-each select="item">
				        <label>
				        	<xsl:attribute name="class">btn<xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text> 
				        		<xsl:value-of select="../@class_radio"/><xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>
								<xsl:if test="../@editable='false'">disabled</xsl:if><xsl:text disable-output-escaping="yes"><![CDATA[ ]]></xsl:text>
								<xsl:if test="@selected='true'">active</xsl:if>
				        	</xsl:attribute>
							<xsl:if test="../@editable='false'"> <!-- bug bootstrap -->
									<xsl:attribute name="onclick">
										 var evt = event ? event:window.event;
			 							 if (evt.stopPropagation)    evt.stopPropagation();
			 							 if (evt.cancelBubble!=null) evt.cancelBubble = true;
...
<xsl:template name="basic_generate_text_field_responsive">
	<xsl:param name="isEncrypted" select="'false'" />
	<xsl:param name="specialClass" select="'null'" />
	<xsl:param name="actionOnEnter" select="'true'" />
	<xsl:param name="margin_padding" select="'true'" />
	...
	<xsl:call-template name="basic_generate_form_component_responsive">
		...
		<xsl:with-param name="isEncrypted"><xsl:value-of select="$isEncrypted"/></xsl:with-param>
		<xsl:with-param name="specialClass"><xsl:value-of select="$specialClass"/></xsl:with-param>
		<xsl:with-param name="actionOnEnter"><xsl:value-of select="$actionOnEnter"/></xsl:with-param>
...
<xsl:template name="basic_generate_form_component_responsive">
	...
	<xsl:call-template name="basic_generate_component_responsive">
		...
		<xsl:with-param name="specialClass"><xsl:value-of select="$specialClass"/></xsl:with-param>
		<xsl:with-param name="style_attr" select="$style_attr" />
		<xsl:with-param name="margin_padding" select="$margin_padding" />
	...
	<xsl:if test="$change_mode=''">			
			...
			<xsl:if test="@refreshForm='true'">
				<xsl:choose>
					...
					<xsl:otherwise>
						goToRefreshForm('do-PartialRefreshForm', '<xsl:value-of select="$fullname"/>', '<xsl:value-of select="action/@obj_provider"/>','<xsl:value-of select="action/@object_owner_id"/>', getLastModal(), '<xsl:value-of select="action/@id_action"/>', null<xsl:if test="action/@object_context_id">,'<xsl:value-of select="action/@object_context_id"/>'</xsl:if>);
					</xsl:otherwise>
				</xsl:choose>
			</xsl:if>
			<xsl:if test="@connect_control and not(@refreshForm='true')">
					runUpKey('<xsl:value-of select="@connect_control"/>');
			</xsl:if>
	...
	<xsl:if test="constraints">
		<script charset="ISO-8859-1">
			...
			register(
				...
				<xsl:if test="@intable='true'">false</xsl:if><xsl:if test="not(@intable) or @intable!='true'"><xsl:value-of select="constraints/@required"/></xsl:if>,
				...
				'<xsl:value-of select="constraints/@align"/>',<xsl:if test="@intable='true'">true</xsl:if><xsl:if test="not(@intable) or @intable!='true'">false</xsl:if>);
		</script>
	...
	<xsl:if test="not(@intable) or @intable!='true'">
		<xsl:if test="registerAsField">
			<script charset="ISO-8859-1">
				...
				]<xsl:if test="constraints/@required='true'">,'<xsl:value-of select="constraints/@description"/>',true</xsl:if>);
			</script>
		</xsl:if>
		<xsl:if test="not(registerAsField) and @editable='true'">
			<script charset="ISO-8859-1">
				registerNoConstraint('<xsl:value-of select="$fullname"/>',['<xsl:value-of select="$fullname"/>']);
			</script>
		</xsl:if>
	...
	<xsl:if test="@onAnyChange">
		<script charset="ISO-8859-1">
			<xsl:if test="@isCalculeOnStart">
				registerStartedScript('<xsl:value-of select="@orden"/>', '<xsl:value-of select="$fullname"/>', "<xsl:value-of select="@onAnyChange"/>; formatear('<xsl:value-of select="$fullname"/>');");
			</xsl:if>
			<xsl:if test="@isCalculeOnAnyChange">
				registerScript('<xsl:value-of select="@orden"/>', '<xsl:value-of select="$fullname"/>', "<xsl:value-of select="@onAnyChange"/>;formatear('<xsl:value-of select="$fullname"/>');");
			</xsl:if>
		</script>
	</xsl:if>
...
<xsl:template name="registerDependencies">
	...
	<xsl:for-each select="dependencies">
		<script charset="ISO-8859-1">registerControlDependency('<xsl:value-of select="$fullname"/>','<xsl:value-of select="$name_prefix"/><xsl:value-of select="@child"/>');</script>
	</xsl:for-each>
</xsl:template>		
...
<xsl:template name="basic_generate_component">
	...
	<xsl:param name="isEncrypted" select="'false'" />
	<xsl:param name="specialClass" select="'null'" />
	<xsl:param name="with_title" select="'false'" />
	...
	<xsl:variable name="className">
		<xsl:choose>
			...
			<xsl:when test="@skin_stereotype and not(@skin_stereotype='')">
				pwr_<xsl:value-of select="@skin_stereotype"/>
			</xsl:when>
			<xsl:otherwise>
			</xsl:otherwise>
		</xsl:choose>
...
<xsl:template name="basic_generate_component_responsive">
	...
	<xsl:param name="margin_padding" select="'true'" />
	<xsl:param name="name"><xsl:value-of select="@name"/></xsl:param>
	<xsl:param name="force_width" select="0" />
	...
	<xsl:attribute name="style">
		<xsl:call-template name="generate_inline_style_responsive">
			<xsl:with-param name="style_attr" select="$style_attr" />
			<xsl:with-param name="margin_padding" select="$margin_padding" />
			<xsl:with-param name="force_width" select="$force_width" />
		</xsl:call-template>
	</xsl:attribute>
	<xsl:choose>
		<!-- IF IT IS A FORM..... -->
...
<xsl:template name="generate_inline_style_responsive">
	...
	<xsl:if test="$force_width=0"><xsl:if test="@width">width:<xsl:value-of select="@width"/>px;</xsl:if></xsl:if>
	<xsl:if test="@foreground">color:<xsl:value-of select="@foreground"/>;</xsl:if>
	<xsl:if test="@background">background-color:<xsl:value-of select="@background"/>;</xsl:if>
	<xsl:if test="@backgroundimage">background-image:url('<xsl:value-of select="@backgroundimage"/>');</xsl:if>
	<xsl:if test="@backgroundrepeat">background-repeat:<xsl:value-of select="@backgroundrepeat"/>;</xsl:if>
