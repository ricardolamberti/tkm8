...
<!-- 	        <link href="vendor/datatables/Select-1.2.4/css/select.bootstrap.min.css" rel="stylesheet"/>
	        <link href="vendor/datatables/Select-1.2.4/css/select.dataTables.min.css" rel="stylesheet"/>
 -->
 			<!-- Graph -->
		    ...
		    <xsl:for-each select="layouts/css">
			    <link rel="stylesheet">
			    	<xsl:attribute name="href">v_<xsl:value-of select="../../@version"/>/<xsl:value-of select="@source"/></xsl:attribute>
			    </link>
			    <link rel="stylesheet">
			    	<xsl:attribute name="href">v_<xsl:value-of select="../../@version"/>/<xsl:value-of select="@source_colors"/></xsl:attribute>
			    </link>
		    </xsl:for-each>
				...
				<xsl:if test="application/@google_pay">
						<script>
							const googlepayEnvironment='<xsl:value-of select="application/@google_pay_environment"/>';
							const googlepayGateway='<xsl:value-of select="application/@google_pay_gateway"/>';
							const googlepayGatewayMerchantId='<xsl:value-of select="application/@google_pay_gatewayMerchantId"/>';
					</script>
					<script async="">
	...
	<!-- 
		    	<script type="text/javascript">
					<xsl:attribute name="src">v_<xsl:value-of select="@version"/>/vendor/font-awesome/js/all.min.js</xsl:attribute>
				</script>
	-->
			    <script type="text/javascript">
...
<!--
			    ...
			    <script src="vendor/sbadmin/data/morris-data.js"></script>
-->
			    <xsl:for-each select="layouts/js">
				    <script rel="stylesheet">
				    	<xsl:attribute name="src">v_<xsl:value-of select="../../@version"/>/<xsl:value-of select="@source"/></xsl:attribute>
				    </script>
			    </xsl:for-each>
				<script type="text/javascript">
...
<!-- 				<script src="vendor/datatables/Select-1.2.4/js/dataTables.select.min.js"/> -->
				<script type="text/javascript">
			...
			<style>
				.se-pre-con {
					...
					height: 100%;
					z-index: 9999;
					background: url(html/images/preloader_2.gif) center no-repeat #fff;
				}
			</style>
		</head>
	</xsl:template>
