...
public final class CommonCache {
    ...
    public static void putString(String key, String value, CacheScope scope, Duration ttl) {
        Objects.requireNonNull(value, "value");
        putBytes(key, value.getBytes(StandardCharsets.UTF_8), scope, ttl);
    }

    /** Retrieve a UTF-8 string value. */
    public static String getString(String key, CacheScope scope) {
        byte[] data = getBytes(key, scope);
        return data == null ? null : new String(data, StandardCharsets.UTF_8);
    }

    /** Store raw bytes. */
    public static void putBytes(String key, byte[] bytes, CacheScope scope, Duration ttl) {
    ...
    /** Retrieve raw bytes. */
    public static byte[] getBytes(String key, CacheScope scope) {
        String fullKey = buildKey(key, scope);
        DistCache cache = CacheProvider.get();
    ...
    public static void remove(String key, CacheScope scope) {
        String fullKey = buildKey(key, scope);
        DistCache cache = CacheProvider.get();
    ...
    public static void clearGlobalPrefix(String prefix) {
    ...
    private static String buildKey(String key, CacheScope scope) {
    ...
    private static String contextId(CacheScope scope) {
        switch (scope) {
        case USER:
    ...
    private static Iterable<String> tryKeysByPrefix(DistCache cache, String prefix) {
        try {
    ...
    private static void addToIndex(CacheScope scope, String ctxId, String fullKey) {
    ...
    private static void removeFromIndex(CacheScope scope, String ctxId, String fullKey) {
        String idxKey = indexKey(scope, ctxId);
        if (idxKey == null) {
            return;
        }
    ...
    private static String indexKey(CacheScope scope, String ctxId) {
        switch (scope) {
        case USER:
    ...
    private static Set<String> readIndex(String idxKey) {
        DistCache cache = CacheProvider.get();
