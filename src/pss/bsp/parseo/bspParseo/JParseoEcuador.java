...
import pss.bsp.pdf.ecu.IFinderEcu;
import pss.bsp.pdf.ecu.JParseoEcu;
import pss.bsp.pdf.ecu.cabecera.BizEcuCabecera;
import pss.bsp.pdf.ecu.cabecera.GuiEcuCabecera;
import pss.bsp.pdf.ecu.detalle.BizEcuDetalle;
import pss.bsp.pdf.ecu.detalle.GuiEcuDetalles;
...
public class JParseoEcuador extends JParseoEcu implements IFinderEcu, IBspSititaParser {

	
	public JParseoEcuador() {
...
//		JBDatos.GetBases().commit();	
	...
	public JWin getGuiHeader(String zCompany, String zOwner, String zIdPdf) throws Exception {
		GuiEcuCabecera cabecera = new GuiEcuCabecera();
...
//		cabecera.GetcDato().addFilter("owner", zOwner);
	...
	public JWins getGuisDetail(String zCompany, String zOwner, String zIdPdf) throws Exception {
		GuiEcuDetalles detalle = new GuiEcuDetalles();
...
//		detalle.getRecords().addFilter("owner", zOwner);
		...
		GuiEcuImpuestos impuestos = new GuiEcuImpuestos();
...
//		impuestos.getRecords().addFilter("owner", zOwner);
	...
	public boolean fillFormat(BizPDF pdfBase,String[][] learningBase,BizFormato formato) throws Exception {
	  JRecords<BizEcuDetalle> details = new JRecords<BizEcuDetalle>(BizEcuDetalle.class);
...
//	  details.addFilter("owner", pdfBase.getOwner());
	  ...
	  JIterator<BizEcuDetalle> it = details.getStaticIterator();
	  int cant=0;
	  ...
	  while (it.hasMoreElements()) {
	  	BizEcuDetalle detail = it.nextElement();
