...
public class FormDetalleUpfront extends JBaseForm {
...
/**
   ...
   */
  public FormDetalleUpfront() throws Exception {
    try { jbInit(); }
...
//    AddItem( pssEdit, DATE, OPT, "fecha_calculo" ).SetValorDefault(getWin().GetcDato().getObjContrato().getFechaHasta()).setRefreshForm(true);
//
//    JFormControl c=AddItem( getJCheckBox(), OPT,"acumulativo");
//    c.SetValorDefault(false);
//    c.SetReadOnly(true);
////    c.setRefreshForm(true);
//    AddItem( getJComboBox(), CHAR, OPT, "periodo" , JWins.createVirtualWinsFromMap(BizDetalleUpfront.getPeriodos())).SetReadOnly(true);
////    AddItem( getValorObjetivo(), FLOAT, OPT, "valor_total" ).setVisible(true);
//    AddItem( getValorObjetivoFC(), FLOAT, OPT, "valor_totalcontrato" ).setVisible(true).SetReadOnly(true);
...
//  panelI.AddItemEdit( "Descripción", CHAR, OPT, "descripcion" ).SetReadOnly(true);
	  ...
	  panelI.AddItemCombo( "Indicador ganancia", CHAR, REQ, "variable_ganancia" , new JControlCombo() {
			@Override
			public JWins getRecords(boolean one) throws Exception {
				return getIndicadorGanancia(one);
			}
	  ...
	  panelI.AddItemDateTime( "Fecha desde cálculo", DATE, OPT, "fecha_desde_calculo" ).SetValorDefault(getWin().GetcDato().getObjContrato().getFechaHasta()).setRefreshForm(true);
