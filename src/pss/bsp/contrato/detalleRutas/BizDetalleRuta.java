...
public class BizDetalleRuta extends BizDetalle {

  
  public BizDetalleRuta() throws Exception {
		super();
	}
  	...
  	//ver borrado
		if (!getMercados().equals("")) {
	  	...
	  	while (toks.hasMoreTokens()) {
	  		...
	  		String sql="select     ";
	  		sql+= " ( case (";
	  		sql+= " ( select  COALESCE(sum(b.cant_roundtrip),0)   from TUR_PNR_BOLETO b WHERE (b.company=  TUR_PNR_BOLETO.company)    AND  b.is_ticket = 'S' AND  b.void = 'N'   AND "+rutaB+"  ";
	  		sql+= strFechasB;
	  		sql+= iatasB;
 				sql+= " )) ";
	  		sql+= " when 0 then 0 else ((100.0* ";
	  		sql+= " COALESCE(sum(TUR_PNR_BOLETO.cant_roundtrip),0) ";
	  		sql+= " / ( ";
	  		sql+= " ( select  COALESCE(sum(b.cant_roundtrip),0)   from TUR_PNR_BOLETO b WHERE (b.company=  TUR_PNR_BOLETO.company)    AND  b.is_ticket = 'S' AND  b.void = 'N'   AND "+rutaB+" ";
	  		sql+= strFechasB;
	  		sql+= iatasB;
	  		...
	  		if (getFareBasicIncludes()!=null) {
	  			...
	  			while (toksTC.hasMoreTokens()) {
	  				...
	  				if (s.trim().indexOf("%")!=-1)
	  					sqlTC += (sqlTC.equals("") ? "" : " OR ") + " (TUR_PNR_SEGMENTOAEREO.fare_basic like '" + s.trim() + "') ";
	  				else
	  					sqlTC += (sqlTC.equals("") ? "" : " OR ") + " (TUR_PNR_SEGMENTOAEREO.fare_basic = '" + s.trim() + "') ";
	  				rutaDef+=(rutaDef.equals("")?"":",")+s.trim();
	  			}
	  			if (!sqlTC.equals(""))
	  				sql += ( " AND ") + "("+ select+"("+ sqlTC+")"+"))";
	  		}
	  		...
	  		sqlg += iatas;
	  		StringTokenizer toksTC;
	  		if (getFareBasicIncludes()!=null) {
	  			String rutaDef="";
	  			String sqlTC = "";
	  			String select = " EXISTS (select TUR_PNR_SEGMENTOAEREO.interface_id from TUR_PNR_SEGMENTOAEREO where TUR_PNR_SEGMENTOAEREO.interface_id=TUR_PNR_BOLETO.interface_id and ";
	  		  toksTC = new StringTokenizer(getFareBasicIncludes(), ",;");
	  			while (toksTC.hasMoreTokens()) {
	  				String s = toksTC.nextToken();
	  				if (s.trim().indexOf("%")!=-1)
	  					sqlTC += (sqlTC.equals("") ? "" : " OR ") + " (TUR_PNR_SEGMENTOAEREO.fare_basic like '" + s.trim() + "') ";
  	...
  	//ver borrado
		if (!getMercados().equals("")) {
	  	...
	  	while (toks.hasMoreTokens()) {
	  		...
	  		String sql="select  coalesce(sum(TUR_PNR_BOOKING.roundtrip),0)  as PORC_COUNT_TUR_PNR_BOLETO_COUNT ";	
...
//    	} 
  ...
  public void processInsert() throws Exception {
  	...
  	setLogicaContrato(JContratoRuta.class.getName());
  ...
  public void processUpdate() throws Exception {
	...
	public String getImagen1(boolean printerVersion) throws Exception {
		GuiObjetivosRutas w = new GuiObjetivosRutas();
		BizObjetivosRutas r = getObjetivos();
		w.setRecords(r);
	...
	public BizObjetivosRutas getObjetivos()  throws Exception {
		BizObjetivosRutas w = new BizObjetivosRutas();
  ...
  public void calcule(boolean update)  throws Exception {
  	BizObjetivosRutas objs=getObjetivos();
		objs.readAll();
  ...
  public String imprimirResumen() throws Exception {
		...
		String tempfile =BizPlantilla.generateListadoTemporario(this.getCompany(),true,recs,"sys_contrato_detalle_rutas");
		return tempfile;
	}
