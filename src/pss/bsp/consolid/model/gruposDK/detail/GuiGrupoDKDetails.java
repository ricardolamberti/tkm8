package  pss.bsp.consolid.model.gruposDK.detail;

import pss.core.win.JWin;
import pss.core.win.JWins;
import pss.core.winUI.lists.JWinList;

public class GuiGrupoDKDetails extends JWins {



  /**
   * Constructor de la Clase
   */
  public GuiGrupoDKDetails() throws Exception {
  }


  public int     GetNroIcono() throws Exception  { return 10036; } 
  public String  GetTitle()    throws Exception  { return "Detalle"; }
  public Class<? extends JWin>  GetClassWin()                   { return GuiGrupoDKDetail.class; }
  /**
   * Mapeo las acciones con las operaciones
   */
  public void createActionMap() throws Exception {
    addActionNew( 1, "Nuevo detalle" );
  }



  /**
   * Configuro las columnas que quiero mostrar en la grilla
   */
  public void ConfigurarColumnasLista(JWinList zLista) throws Exception {
    zLista.AddIcono("");
    zLista.AddColumnaLista("dk");
    zLista.AddColumnaLista("view_all");

  }


}
