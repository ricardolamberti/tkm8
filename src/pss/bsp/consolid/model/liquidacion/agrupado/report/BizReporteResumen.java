package pss.bsp.consolid.model.liquidacion.agrupado.report;
...
public class BizReporteResumen extends JBDReportes  {
  ...
  protected JReport obtenerReporte() throws Exception {
  	String sPath = "";
  	sPath = "bsp/consolid/model/liquidacion/agrupado/report/resumenAgrup.jasper";
      ...
      public static void main(String[] args) {
          try {
              ...
              JasperReport reporteCompilado = (JasperReport) JRLoader.loadObject(new File("/dev/tkm8/tkm8/src/pss/bsp/consolid/model/liquidacion/agrupado/report/resumenAgrup.jasper"));
  ...
  protected void configurarControles() throws Exception {
  	...
  	if (BizBSPCompany.getObjBSPCompany( BizUsuario.getUsr().getCompany()).isDependant()) {
  			String logo = BizBSPCompany.getObjBSPCompany(BizBSPCompany.getObjBSPCompany( BizUsuario.getUsr().getCompany()).getParentCompany()).getLogo();
    ...
    getReport().setImage("LOGO_INC", logoFile);
  	...
  	getReport().setLabel("LBL_DK", getHeader().getObjGrupoDK().getIdentificador());
	...
	private BizLiqAgrupado oLiqAcum = null;
	...
	public JRecords<BizLiqDetail> getRecords() throws Exception {
		...
		recs.addJoin(JRelations.JOIN, "BSP_GRUPO_DK_DETAIL", "BSP_GRUPO_DK_DETAIL.dk=BSP_LIQUIDATION_DETAIL.dk and BSP_GRUPO_DK_DETAIL.company=BSP_LIQUIDATION_DETAIL.company");
		recs.addJoin(JRelations.JOIN, "BSP_GRUPO_DK", "BSP_GRUPO_DK.id_grupo=BSP_GRUPO_DK_DETAIL.id_grupo and BSP_GRUPO_DK.company=BSP_GRUPO_DK_DETAIL.company");
		recs.addFilter("BSP_GRUPO_DK", "id_grupo", pDK.getValue(),"=");
	...
	public BizLiqAgrupado getHeader() throws Exception {
		if (oLiqAcum!=null) return oLiqAcum;
		BizLiqAgrupado rec = new BizLiqAgrupado();
		rec.addFilter("grupo_dk", pDK.getValue());
