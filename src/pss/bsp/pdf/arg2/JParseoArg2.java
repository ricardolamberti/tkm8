...
public class JParseoArg2 implements IParseo {
	...
	BizArgImpuesto impuesto;
	BizArgImpuesto impuesto2;
	BizArgImpuesto impuesto3;
	...
	List<BizArgImpuesto> impuestos = null;
	...
	public void addListenerDetect(IFinder finder) {
		this.finder = (IFinderArg2) finder;
	}
...
//		}
		...
		impuesto = new BizArgImpuesto();
		impuesto2 = new BizArgImpuesto();
		impuesto3 = new BizArgImpuesto();
		boolean tarjeta = false;
		boolean ex = false;
		boolean shortest = false;
	  ...
	  PssLogger.logInfo("PRE PROCESO LINEA : " + linea2);
		...
		while (!exit && fieldIdx != LINE_FIELDS.END) {
			...
			if (shortest && fieldIdx.equals(LINE_FIELDS.FIELD_CODIGO_CPN))
				break;
...
// 			switch (c) {
			...
			case FIELD_NRO_DE_DOC:  // NO. DE DOCUMENTO (Boleto)
				if (sCont.equals("3328798308"))
					PssLogger.logInfo("find ticket");
				boleto = sCont;
...
// 			c++;
	...
	public void startNewDetail() throws Exception {
		...
		impuestos_contado = 0;
		impuestos_tarjeta = 0;
		impuestos = new ArrayList<BizArgImpuesto>();
		nextLinea++;
	}
...
//			if (actual_operacion.indexOf("TICKET")!=-1 && actual_aerolinea.indexOf("IATA")==-1) {
			readyDetail();
			for (BizArgImpuesto impuesto : impuestos)
...
//			}
	...
	public boolean detectTax(String linea) throws Exception {
		...
		Pattern p = Pattern.compile(
				...
				"(\\p{Blank}*)(((\\p{Digit}|\\p{Punct})*)\\p{Blank})((\\p{Alpha}|\\p{Digit}){2})"
				+ "|(\\p{Blank}*)((\\p{Digit}|\\p{Punct})*)(\\p{Blank})((\\p{Alpha}|\\p{Digit}){2})(\\p{Blank}*)((\\p{Digit}|\\p{Punct})*)(\\p{Blank})((\\p{Alpha}|\\p{Digit}){2})"
							);
		Matcher m = p.matcher(linea);
		boolean b = m.matches();
		if (!b)
	...
	private void readyTax() throws Exception {
		...
		for (BizArgImpuesto imp:impuestos) {
			if (imp.getIso().equals(impuesto.getIso())) {
				repetido1=true;
				if (imp.getIso().equals("QN")) {
					qn = impuesto.getContado() + impuesto.getTarjeta();
				}
				imp.setContado(impuesto.getContado());
				imp.setTarjeta(impuesto.getTarjeta());
			...
			if (imp.getIso().equals(impuesto2.getIso())) {
				...
				imp.setContado(impuesto2.getContado());
				imp.setTarjeta(impuesto2.getTarjeta());
				impuestos_contado += impuesto2.getContado();
				impuestos_tarjeta += impuesto2.getTarjeta();
			}
		}

		if (!impuesto.getIso().equals("") && !repetido1) {
		...
		impuesto = new BizArgImpuesto();
		if (!impuesto2.getIso().equals("") && !repetido2) {
...
//			finalizar=true;	
	...
	private void detectMoneda(String linea) throws Exception {
		if (linea.trim().startsWith("ARG")&&linea.trim().endsWith("ARS"))
			lastMoneda=("ARS");
		else if (linea.trim().startsWith("ARG")&&linea.trim().endsWith("USD"))
			lastMoneda=("USD");
		else 
			return;
		cabecera.setMoneda("ARS");
		if (detectMoneda) {
			detectMoneda= false;
			readyHeader();
		}
	...
	public static void main(String[] args) {
		JParseoArg2 parser = new JParseoArg2();
		IFinderArg2 finder = new IFinderArg2() {
