...
public class JParseoCRi implements IParseo {
	...
	public String getTableDetalle() throws Exception {
		return new BizCRiDetalle().GetTable();
	}
	...
	BizCRiCabecera cabecera;

	BizCRiDetalle detalle;

	BizCRiImpuesto impuesto;
	BizCRiImpuesto impuesto2;
	...
	List<BizCRiImpuesto> impuestos = null;
...
//		if (detectTax(linea)) return;
	...
	private void detectSeccion(String linea) throws Exception {
		if (linea.indexOf("ANALISIS DE TRANSACCIONES POR AGENCIAS DE VIAJES") == -1) return;
		detectSeccion = false;
	}
...
//	}
	...
	public void startNewDetail() throws Exception {
		...
		impuestos_contado = 0;
		impuestos_tarjeta = 0;
		impuestos = new ArrayList<BizCRiImpuesto>();
		nextLinea++;
	}
...
//			if (actual_operacion.indexOf("TICKET")!=-1 && actual_aerolinea.indexOf("IATA")==-1) {
				readyDetail();
				for (BizCRiImpuesto impuesto : impuestos)
...
//	}
	...
	private void readyTax() throws Exception {
			...
			impuesto2 = new BizCRiImpuesto();
			impuesto2.setCompany(company);
			impuesto2.setOwner(owner);
			impuesto2.setIdpdf(idPdf);
			impuesto2.setLinea(nextLinea);
			impuesto2.setContado(detalle.getContado() == 0 ? 0 : detalle.getImpuesto2());
			impuesto2.setTarjeta(detalle.getContado() != 0 ? 0 : detalle.getImpuesto2());
			impuesto2.setIso("NL");
	...
	private void readyDetail() throws Exception {
		...
		if (Math.abs(suma) < Math.abs(detalle.getTotal()))
			detalle.setContadoBruto(detalle.getContadoBruto() );
		else 
			detalle.setTarjetaBruto(detalle.getTarjetaBruto() );
	...
	private void detectPeriodo(String linea) throws Exception {
		...
		Pattern p = Pattern.compile("R2030(\\p{Blank})++\\d\\d\\/\\d\\d\\/\\d\\d(\\p{Blank})++DE(\\p{Blank})++\\d\\d\\/\\d\\d\\/\\d\\d(\\p{Blank})++AL(\\p{Blank})++\\d\\d\\/\\d\\d\\/\\d\\d");
		Matcher m = p.matcher(linea);
		boolean b = m.find();

		if (!b) return;
		int start, s1, s2;
		if ((start = linea.indexOf("DE")) == -1) return;
	...
	public static void main(String[] args) {
		JParseoCRi parser = new JParseoCRi();
		IFinderCRi finder = new IFinderCRi() {
