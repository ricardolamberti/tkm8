...
public class JParseoArg implements IParseo {
	...
	protected void parserLinea(String linea) throws Exception {
		...
		if (detectAerolinea(linea)) return;
	...
	public boolean detectLinea(String linea) throws Exception {
		...
		closePendingDetail();
		startNewDetail();
		int[] columnas = { 0, 10, 13, 17, 24, 35, 49, 52, 61, 72, 83, 93, 102, 112, 122, 133, 143 };
		String boleto = "";
...
//		}
	...
	private void detectId(String linea) throws Exception {
		int start;
		if ((start = linea.indexOf("Id::")) == -1) return;
		int stop = linea.indexOf(" ", start);
		String id = linea.substring(start + 4, stop);
	...
	private void detectHeader(String linea) throws Exception {
		...
		if (s+132>linea.length()) 
			detectHeaderSingle(linea.substring(s));
		else
			detectHeaderSingle(linea.substring(s,s+132));
		} 
	}
	...
	private void detectPeriodo(String linea) throws Exception {
		...
		Pattern p = Pattern.compile("\\|(\\p{Blank})++DE(\\p{Blank})++\\d\\d\\/\\d\\d\\/\\d\\d(\\p{Blank})++AL(\\p{Blank})++\\d\\d\\/\\d\\d\\/\\d\\d(\\p{Blank})++\\|");
		Matcher m = p.matcher(linea);
		boolean b = m.find();
		...
		if ((s2 = linea.indexOf("|", s1)) == -1) return;

		fechaDesde = getFecha(linea.substring(start + 4, s1).trim());
		fechaHasta = getFecha(linea.substring(s1 + 4, s2).trim());
	...
	private void detectMoneda(String linea) throws Exception {
		...
		cabecera.setMoneda(linea.substring(start + 8, s1).trim());
	...
	public static void main(String[] args) {
		JParseoArg parser = new JParseoArg();
		IFinderArg finder = new IFinderArg() {
