...
public class JParseoGua implements IParseo {
	...
	public String getTableDetalle() throws Exception {
		return new BizGuaDetalle().GetTable();
	}
	BizGuaCabecera cabecera;

	BizGuaDetalle detalle;

	BizGuaImpuesto impuesto;
	...
	List<BizGuaImpuesto> impuestos = null;
...
//		if (detectTax(linea)) return;
	...
	private void detectSeccion(String linea) throws Exception {
		if (linea.indexOf("ANALISIS DE TRANSACCIONES") == -1) return;
		detectSeccion = false;
	}
...
//	}
	...
	public void startNewDetail() throws Exception {
		...
		impuestos_contado = 0;
		impuestos_tarjeta = 0;
		impuestos = new ArrayList<BizGuaImpuesto>();
		nextLinea++;
	}
...
//			if (actual_operacion.indexOf("TICKET")!=-1 && actual_aerolinea.indexOf("IATA")==-1) {
				readyDetail();
				for (BizGuaImpuesto impuesto : impuestos)
...
//	}
	...
	private void detectHeaderSingle1(String linea) throws Exception {
		...
		boolean b = linea.indexOf("AGENCIA") != -1;
		boolean bb = linea.indexOf("ANALISIS DE TRANSACCIONES POR AGENCIAS DE VIAJES") != -1;
	...
	private void detectPeriodo(String linea) throws Exception {
		...
		Pattern p = Pattern.compile("DE(\\p{Blank})++\\d\\d\\/\\d\\d\\/\\d\\d(\\p{Blank})++AL(\\p{Blank})++\\d\\d\\/\\d\\d\\/\\d\\d");
		Matcher m = p.matcher(linea);
		boolean b = m.find();
	...
	public static void main(String[] args) {
		JParseoGua parser = new JParseoGua();
		IFinderGua finder = new IFinderGua() {
