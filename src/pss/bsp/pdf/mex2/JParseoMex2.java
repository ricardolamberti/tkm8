...
public class JParseoMex2 implements IParseo {
	...
	int lineaMulti=0;
	List<BizMexImpuesto> impuestos = null;
	...
	protected void parserLinea(String linea) throws Exception {
		...
		if (detectLinea2(linea)) return;
		if (detectTax(linea)) return;
	}

	private void detectSeccion(String linea) throws Exception {
		if (!linea.trim().startsWith("CATEGORY")) return;
		if (!linea.trim().endsWith("BSP")) return;
		detectSeccion = false;
	}
	...
	public boolean detectLinea(String linea) throws Exception {
	  ...
	  impuesto = new BizMexImpuesto();
	  impuesto2 = new BizMexImpuesto();
	  impuesto3 = new BizMexImpuesto();
	  ...
	  if (linea2.indexOf("6977038168")!=-1)
	  	PssLogger.logDebug("test");
...
//				detalle.setTotal(getDouble(sCont));
			...
			if (c==13) {
				...
				if (getDouble(sCont)==detalle.getTarjeta()+detalle.getContado()) {
					c+=2;
				}else {
	...
	public boolean detectLinea2(String linea) throws Exception {
	  ...
	  boolean shortest=false;
		...
		//** 374.31  53.56 44.40 AJ 175.00 YQ  50.35 6.00 3.21 0.00  0.00  371.10
	  String linea2=linea.replaceAll("  "," ? ");
	  if (linea2.indexOf("6977038168")!=-1)
	  	PssLogger.logDebug("test");
		StringTokenizer toks = new StringTokenizer(linea2," ");
		...
		while (!exit) {
			...
			if (c==4) {
				if (!tarjeta)
					detalle.setTarjeta(getDouble(sCont)-detalle.getContado());
		  	else
					detalle.setContado(getDouble(sCont)-detalle.getTarjeta());
			}
			if (c==5) {
	...
	public void startNewDetail() throws Exception {
		...
		impuesto2 =null;
		impuesto3 =null;
		impuestos = new ArrayList<BizMexImpuesto>();
		nextLinea++;
	}
	...
	public void closePendingDetail() throws Exception {
		if (pendingDetail) {
			readyTax();
			readyDetail();
			for (BizMexImpuesto impuesto : impuestos)
...
*/
	...
	private void detectId(String linea) throws Exception {
		...
		String id = JTools.removeSpaces(linea.substring(start + 10).trim());
		if (id.length() > 20) return;
		idPdf = JTools.getNumberEmbedded(id);
		cabecera.setIdpdf(idPdf);
		cabecera.setFullData(false);
		readyHeader();
	}
	...
	private void detectHeader(String linea) throws Exception {
		...
		if (b) {
		    int pos =linea.indexOf("REFERENCE:")+10;
				String id = JTools.removeSpaces(linea.substring(pos).trim());
			iata = id.substring(id.indexOf("-")+1).trim();
	...
	private void detectPeriodo(String linea) throws Exception {
		...
		if ((s1 = linea.substring(0,start).lastIndexOf("(")) == -1) return;
		if ((s2 = linea.indexOf(")", start)) == -1) return;

		fechaDesde = getFechaDDMMMAA(linea.substring(s1+1, start).trim());
		fechaHasta = getFechaDDMMMAA(linea.substring(start+4,s2).trim());
...
//		if ((s1 = linea.indexOf("    ", start)) == -1) return;
	...
	private Date getFecha(String linea) throws Exception {
	 ...
	 public  Date getFechaDDMMMAA(String linea) throws Exception {
	  	...
	  	String fecha = linea.substring(start + 1, s1).trim();
			...
			if (fecha.indexOf("JAN")!=-1)      mes=Calendar.JANUARY;
	...
	public static void main(String[] args) {
		JParseoMex2 parser = new JParseoMex2();
		IFinderMex2 finder = new IFinderMex2() {
