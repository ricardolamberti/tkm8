...
//		oConfig = new RConfig();
	...
	public static void Exit() {
		try {
			PssLogger.logWait("Cerrando aplicaci�n...");
			GetApp().shutdownApp();
			GetApp().closeApp();
			GetApp().ExitApp();
		} catch (Exception ex) {
			PssLogger.logInfo("No se pudo cerrar la aplicaci�n");
...
//		  JPssVersion.setPssVersion(version);
	...
	public void closeApp() throws Exception {
		...
		try {
			...
			PssLogger.logInfo("No se cierra la Base de Datos porque no hay conexi�n");
		}
	}
