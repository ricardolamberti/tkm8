...
public class GuiReportPreview extends JWin {
  ...
  private static final int ICON_DHTML   = 927;
  ...
  @Override
    ...
    addAction(12, "Exportar a HTML", new JAct() {
      @Override
			public void Do() throws Exception {
        if (!setOutputFilenameInReport("Exportar a HTML", ICON_HTML, "html")) return;
        GetcDato().exportToHTML();
      }
    }, ICON_HTML, false, true );

    addAction(13, "Exportar a HTML dinámico", new JAct() {
      @Override
			public void Do() throws Exception {
        if (!setOutputFilenameInReport("Exportar a HTML dinámico", ICON_DHTML, "html")) return;
        GetcDato().exportToDHTML();
      }
    }, ICON_DHTML, false, true );

    addAction(14, "Exportar a Excel", new JAct() {
      @Override
			public void Do() throws Exception {
        if (!setOutputFilenameInReport("Exportar a Excel", ICON_EXCEL, "xls")) return;
        GetcDato().exportToExcel();
      }
    ...
    addAction(15, "Exportar a PDF", new JAct() {
      @Override
			public void Do() throws Exception {
        if (!setOutputFilenameInReport("Exportar a PDF", ICON_PDF, "pdf")) return;
        GetcDato().exportToPDF();
      }
    }, ICON_PDF, false, true );
  ...
  private boolean setOutputFilenameInReport(String zDialogTitle, int zIconNumber, String zExtension) throws Exception {
    String sFileName;
    try {
      if ((sFileName = askForFileNameIfNeeded(zDialogTitle, zIconNumber, zExtension)) != null
          && checkFileOverwrite(sFileName)) {
        GetcDato().getReport().SetPath(sFileName);
        return true;
      }
  ...
  private String askForFileNameIfNeeded(String zDialogTitle, int zIconNumber, String zExtension) throws Exception {
    ...
    if (!bForceConfiguredExportDir) {
      sConfiguredExportDir = GetcDato().getReport().getToolBarConfiguration().getOutputDirectory();
    } else {
