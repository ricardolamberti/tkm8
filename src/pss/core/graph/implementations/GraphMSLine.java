...
public class GraphMSLine extends GraphMatrix {
	...
	public GraphMSLine(String title) {
		super(title);
	}
		...
		boolean largo = false;
			...
			while (ids.hasMoreElements()) {
				...
				while (iv.hasMoreElements()) {
					...
					dataset.setValue(v.getData().toString().equals("0.0")?null:Float.parseFloat(v.getData().toString()),ds.getColname(),v.getCategorie());
					largo |= v.getCategorie().length()>=4;
			  }		
			}
		  
			JFreeChart chart = ChartFactory.createLineChart(getTitle(), getAtributtes().getElement(GRAPH_ATTR_XAXISNAME), 
