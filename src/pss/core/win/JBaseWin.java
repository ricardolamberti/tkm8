...
//  private String statusHelp;
	...
	public boolean isCard() {
		if (getDropControlIdListener() == null)
			return false;
	...
	public String getSwapDescripField() {
		return getDescripField();
	}
	...
	public void SetBaseDato(JBaseRecord zValue) throws Exception {
		oBaseDato = zValue;
		if (oBaseDato != null)
			this.SetVision(oBaseDato.GetVision());
	}
	...
	public String getColor() throws Exception {
		String Colors[] = BizUsuario.getSkin().createGenerator().getColorGraph();// {
																																							// "a6cee3","1f78b4","b2df8a","33a02c","fb9a99","e31a1c","fdbf6f","ff7f00","cab2d6","6a3d9a","ffff99","b15928","8dd3c7","ffffb3","bebada","fb8072","80b1d3","fdb462","b3de69","fccde5","d9d9d9","bc80bd","ccebc5","ffed6f","1b9e77","d95f02","7570b3","e7298a","66a61e","e6ab02","a6761d","666666","fbb4ae","b3cde3","ccebc5","decbe4","fed9a6","ffffcc","e5d8bd","fddaec","f2f2f2"
																																							// };
	...
	public String getSearchFields(boolean withKey, String zkey, String zdescrip) throws Exception {
		...
		if (key == null)
			key = this.getSearchKeyFields();
		if (descrip == null)
			descrip = this.getSearchFields();
	...
	public boolean isVision(String zVision) {
		if (zVision == null && GetVision() == null)
			return true;
		if (zVision != null && GetVision() == null)
			return false;
		if (zVision == null && GetVision() != null)
...
//	}
	...
	public BizAction addAction(int zId, String zIdAction, BizAction zAction, String zDescr, JAct zActionLis, int ziKeyAction, int zIcon, boolean zEnMenu, boolean zEnBar, boolean zEnArbol, String zTipoArbol, boolean cancelable, String specialSelector) throws Exception {
		...
		if (zAction == null)
			action = this.addAction(oAct);
		else
			action = this.addAction(zAction, oAct);
...
//	}
	...
	public BizAction findActionByUniqueId(String zActId, boolean security, boolean okAction) throws Exception {
		...
		if (as == null)
			JExcepcion.SendError("action map en nulo");
		BizAction a = as.findActionByUniqueId(zActId);
		if (a == null)
			return null;
		if (security && !a.isOkSecurity())
			return null;
		if (okAction && !a.isOkAction())
			return null;
		return a;
	}
...
//	}
	...
	/**
	 * Serializes the BizActions that this BaseWin object handles and, if
	 * bRecursive=true, the subaactions of each element, enclosing them in <item>
	 * tags.
	 */
	/*
	 * public void WebGenerarAccionesMenu(JSAXWrapper wrapper, boolean bRecursive,
	 * boolean bWeb) throws Exception { BizActions acciones = this.getActionMap();
	 * JInteger oID = new JInteger(0); acciones.firstRecord(); while
	 * (acciones.nextRecord()) { WebGenerarAcciones(wrapper, bRecursive, (BizAction)
	 * acciones.getRecord(), false, true, bWeb, oID); } }
	 * 
	 * private void WebGenerarAcciones(JSAXWrapper wrapper, boolean bRecursive,
	 * BizAction jbaccion, boolean bToolbar, boolean bMenu, boolean bWeb, JInteger
	 * zID) throws Exception { boolean validada = true; AttributesImpl attr = new
	 * AttributesImpl();
	 * 
	 * if (bToolbar && (!jbaccion.ifToolBar() && !jbaccion.ifArbol())) { validada =
	 * false; }
	 * 
	 * if (bMenu && !jbaccion.ifMenu()) { validada = false; } // if( bWeb &&
	 * !jbaccion.ifToolBar() ) { // validada = false; // }
	 * 
	 * if (validada) { // Si el pedido es recursivo encierra las acciones en
	 * elementos // "item" if (bRecursive) { attr.clear(); attr.addAttribute("",
	 * "id", "id", "CDATA", zID.toString()); wrapper.startElement("item", attr);
	 * zID.setValue(zID.getValue() + 1); }
	 * 
	 * jbaccion.toSAX(wrapper);
	 * 
	 * if (bRecursive) { if (jbaccion.hasSubActions()) { // Subacción
	 * jbaccion.GetSubAcciones().firstRecord(); while
	 * (jbaccion.GetSubAcciones().nextRecord()) { BizAction subaccion = (BizAction)
	 * jbaccion .GetSubAcciones().getRecord(); WebGenerarAcciones(wrapper,
	 * bRecursive, subaccion, bToolbar, bMenu, bWeb, zID); } }
	 * 
...
//		clearActions();// el tema es que no encuentra el drop
	...
	/*
	 * public JBaseWin getExpandedItems(JBaseWin zOrigen) throws Exception {
	 * JIterator iter = this.getTreeActions().getIterator(); while (
	 * iter.hasMoreElements() ) { GuiAction oAction = (GuiAction)
	 * iter.nextElement(); if ( ! oAction.GetcDato().ifArbolDetail() ) continue;
	 * return oAction.GetcDato().GetAction().getResult(); } return null; }
	 * 
	 */
	...
	public boolean refreshOnSubmit() throws Exception {
		if (refreshOnSubmit == 0)
			return this.canConvertToURL(); // si esta en la base de datos, si esta en memoria no
	...
	// Reimplementar si se desea cambia las zonas estandar de JwinList JwinMatrix,
	// el resultado de esta funcion se enviara como vision en el drop
	...
	// lista de clases con simpleClassname separado por espacios que se aceptan en
	// el drop, null = todas
	...
	public String getHtmlView(int action, String builderArguments, JFilterMap params, boolean convertScriptRefToFile, boolean convertScriptRefToPrefix, boolean convertInjectStyle, boolean convertImageRef) throws Exception {
		...
		HtmlPayload payload = internalReportService.buildHtmlForOwner(this, "win_list_x", variant, user, filters, xsltParams, null);
	...
	public boolean canEjecuteDrop() throws Exception {
		if (this.hasDropControlIdListener())
			return false;
		return true;
	}
	...
	public boolean canEjecuteDropControl(JAct act) throws Exception {
		...
		if (act == null)
			return true;
		if (this.getDropControlIdListener() == act.getResult().getDropControlIdListener())
			return false;
		return true;
...
//		noActionMapToolbar=null;
	...
	// funciones que se disparan en el recorrido, de la generacion de la grilla, se
	// habilitan los siguientes metodos para hacer calculos y no tener que hacer dos
	// recorridas
	...
	// devuelve customInfo que se cargan al json y pueden ser usados como datos para
	// los updateControls
