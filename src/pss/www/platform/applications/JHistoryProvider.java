...
//		return new JWebWinFactory(null).convertURLToAction(this.action);
	...
	public boolean refreshOnSubmit() throws Exception {
		if (!this.getAction().hasSubmit())
			return false;

		JAct submit = this.getActionSubmit();
		if (!submit.hasResult())
			return true;
		if (!submit.refreshOnSubmit())
			return false; // estoy en un jactnew, navego con un boton, al volver si borro el submit pierdo
										// el unico lugar donde quedo lo almacenado lo ingresado por el usuario
