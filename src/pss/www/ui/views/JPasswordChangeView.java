...
public class JPasswordChangeView extends JWebView {
  ...
  //  this.setTitle("Cambio de contrase�a");
  //  this.setIcon(JWebIcon.getPssIcon(GuiIcon.LOGIN_ICON));
    ...
    // create the form
		...
		title = new JWebHResponsive(3,JLanguage.translate("Cambio contrase�a."));
	  ...
	  oUsuario.setEditable(false);
		...
		oPassword.setLabelLateral(JLanguage.translate("Contrase�a actual"));
		fieldset.add("password",oPassword);

		oPasswordNew = new JWebViewPasswordResponsive();
		oPasswordNew.setLabelLateral(JLanguage.translate("Contrase�a nueva"));
		fieldset.add("new_password",oPasswordNew);

		oPasswordNewConf = new JWebViewPasswordResponsive();
		oPasswordNewConf.setLabelLateral(JLanguage.translate("Contrase�a nueva"));
...
//		oPassword.setController(new JPassword(), "Contrase�a actual",  false, BizSegConfiguracion.C_MAX_PASSWORD_LEN, null);
		oPasswordNew.setController(new JPassword(), "Contrase�a nueva",  false, BizSegConfiguracion.C_MAX_PASSWORD_LEN, null);
		oPasswordNewConf.setController(new JPassword(), "Contrase�a nueva confimaci�n",  false, BizSegConfiguracion.C_MAX_PASSWORD_LEN, null);
  ...
  @Override
    ...
    if (sPassword==null || sPassword.trim().length() < 1) {
      ...
      this.title.setLabel("Ingreso inicial de contrase�a");
      this.oMsgLabel.setLabel("Su contrase�a est� en blanco. Por favor, ingrese una nueva para poder iniciar sesión.");
      oPassword.setController(new JPassword(), JLanguage.translate("Contrase�a actual"), false, BizSegConfiguracion.C_MAX_PASSWORD_LEN, null);
    } else {
    	this.title.setLabel("Cambio obligatorio de contrase�a");
      this.oMsgLabel.setLabel("Su contrase�a ha caducado. Por favor, ingrese una nueva para poder iniciar sesión.");
      oPassword.setController(new JPassword(), JLanguage.translate("Contrase�a actual"), true, BizSegConfiguracion.C_MAX_PASSWORD_LEN, null);
    }
