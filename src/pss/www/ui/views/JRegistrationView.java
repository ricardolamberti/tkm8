...
//		fieldset.add("Pa�s",this.oPais);

		oPasswordNew = new JWebViewPasswordResponsive();
		oPasswordNew.setLabelLateral(JLanguage.translate("Contrase�a"));
		fieldset.add("new_password",oPasswordNew);
		oPasswordNewConf = new JWebViewPasswordResponsive();
		oPasswordNewConf.setLabelLateral(JLanguage.translate("Re-ingrese Contrase�a"));
...
//		});
	...
	public JWebActionResult registration(JAbstractLoginResolver loginResolver) throws Throwable {
		...
		try {
			...
			if (track==null) {
				throw new Exception("Acceso inv�lido");
			}
	...
	public JWebActionResult perform(JAbstractLoginResolver loginResolver) throws Throwable {
		...
		if (!sPassword.equals(sPasswordRepeat)) {
			throw new Exception(getMessage("Contrase�as no coinciden"));
		...
		return loginResolver.getRedirector().goShowMessage(JWebViewsConstants.MESSAGE_TYPE_INFO, "Registraci�n", "Se envi� un mail con el link de acceso");
	}

	// LOGIN_WELCOME_TITLE=Bienvenido
	// LOGIN_WELCOME_TITLE_LOGO=15000
	// LOGIN_WELCOME_TEXT=Ud ha ingresado al �rea privada del SITI. Ingrese los
	...
	private void analizeIniConfiguration() throws Exception {
		...
		loginText = cfg.getCachedValue(secc, "LOGIN_WELCOME_TEXT",
				"Ud ha ingresado al �rea privada del SITI. Ingrese los datos de su usuario y presione 'confirmar' para comenzar a administrar su Empresa");
