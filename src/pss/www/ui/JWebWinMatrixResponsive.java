 ...
 * representation and provides helper methods used by the responsive framework.
 */
...
//					rowWinObjectId = this.rowToXMLActionBar(getNotebookParent() + "" + this.getWins().getClass().getSimpleName() + "_" + iRowPos, zContent, oData.getWin());
			...
			if (oData == null) {
				...
				if (value != null)
					zContent.setAttribute("class_responsive", value);
...
//					zContent.setAttribute("data_xls", sResult); // ver
				...
				if (oData.getWin() != null) {
					if (oData.getWin().isWordWrap(columna.getCampo()))
						zContent.setAttribute("word_wrap", oData.getWin().isWordWrap(columna.getCampo()));
					...
					if (columna.getCampo() != null) {
						...
						if (value != null)
							zContent.setAttribute("foreground", value);
						...
						if (value != null)
							zContent.setAttribute("background", value);
	...
	void rowToXML(JXMLContent zContent, JFilaMatrix fila, int iRowPos, JWins iOldSelected, String iOldSingleSelect) throws Exception {
		...
		while (itColumna.hasMoreElements()) {
			...
			if (--salto == 0)
				salto = fieldToXML(zContent, fila, columna, iRowPos, col, posCol, oldSelect, oldSingleSelect);
			col++;
			posCol += salto;
		}
		if (!isReadOnly()) {
	...
	protected void footToXML(JXMLContent zContent) throws Exception {
		if (!getWins().matrixDisplayTotales())
			return;
...
//		if (isPreview) {
//			getWins().getRecords().setOpts(RegQueryOptions.previewDefaults());
//		} else {
...
//		}
		try {
			...
			if (e.getMessage().indexOf("canceling statement due to user request")!=-1) {
				zContent.startNode("error");
				zContent.setAttribute("message", "Reporte muy extenso, por favor agregue filtros que limiten la busqueda");
				zContent.endNode("error");
			}
	...
	SortedMap<String, JColumnaMatrix> calculateMatrixWithingData() throws Exception {
		...
		if (winMatrix.GetEjesMatrix() == null)
			return map;
...
//		if (idColumna.equals("")) idColumna = ""+(col++);
	 ...
	 * {@link WebMatrixCalculo}. This method prepares the data so it can be rendered
	 * by the responsive view.
	 *
	 ...
	 * assembling the row and column structures of the matrix. It keeps track of
	 * totals, groups and other intermediate data required for rendering.
	 */
...
//				PssLogger.logInfo("FILA: "+idFilaDec);
		...
		boolean isContable(JColumnaLista dato, JObject value) throws Exception {
			return (!dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_LIT) && !dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_ANIO) && !dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_ANOMES) && !dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_ANOSEM) && !dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_BIMESTRE) && !dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_TRIMESTRE) && !dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_CUATRIMESTRE) && !dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_SEMESTRE) && !dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_DIA_MES) && !dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_DIA_ANO) && !dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_DIA_SEMANA) && !dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_MES) && value != null && !value.toString().equals("")
					&& ((dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_COUNT) || dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_AVR) || dato.isPorcentaje() || dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_SUM) || (value.isCurrency() || value.isFloat() || value.isInteger() || value.isLong() || (value.isString() && ((JString) value).getFloatValue() != 0)))));
		}
		...
		void processAcumuladores(JWin win, JColumnaMatrix colMatrix, JDataMatrix data, JColumnaLista dato, JObject value, double actualValue) throws Exception {
			...
			if (dato.isPorcentaje())
				colMatrix.setValor(colMatrix.getValor() + thisValue);
			else if (dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_MAX))
				colMatrix.setValor(colMatrix.getValor() > thisValue ? colMatrix.getValor() : thisValue);
			else if (dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_MIN))
				colMatrix.setValor(colMatrix.getValor() < thisValue ? colMatrix.getValor() : thisValue);
			else if (dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_SUM))
				colMatrix.setValor(colMatrix.getValor() + thisValue);
			else if (dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_COUNT))
				colMatrix.setValor(colMatrix.getValor() + thisValue);
			else if (dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_AVR))
				colMatrix.setValor(colMatrix.getValor() + thisValue);
			else
				colMatrix.setValor(colMatrix.getValor() + thisValue);
			if (dato.isMarcaMayores() && dato.isMarcaTop10())
				select10(dato.getTop10(), data, true);
			if (dato.isMarcaMenores() && dato.isMarcaBottom10())
				select10(dato.getBottom10(), data, false);
			if (dato.isMarcaMayores() && !dato.isMarcaTop10() && !dato.getMayoresA().equals("") && JTools.isNumber(dato.getMayoresA())) {
				if (Float.parseFloat(value.toString()) >= Float.parseFloat(dato.getMayoresA()))
					data.setColorBackground(dato.getColorTopBackground());
			}
			if (dato.isMarcaMenores() && !dato.isMarcaBottom10() && !dato.getMenoresA().equals("") && JTools.isNumber(dato.getMenoresA())) {
				if (Float.parseFloat(value.toString()) <= Float.parseFloat(dato.getMenoresA()))
					data.setColorBackground(dato.getColorBottomBackground());
			}
		...
		void processAcumuladoresTotales(JColumnaMatrix colMatrixT, JDataMatrix dataTotales, JColumnaMatrix colMatrix, JColumnaLista dato, JObject value) throws Exception {
			if (dato.isPorcentaje())
				dataTotales.setAcumulador(dataTotales.getAcumulador() + dataTotales.getValorOrdenador());
			else if (dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_MAX))
				dataTotales.setAcumulador(dataTotales.getAcumulador() > dataTotales.getValorOrdenador() ? dataTotales.getAcumulador() : dataTotales.getValorOrdenador());
			else if (dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_MIN))
				dataTotales.setAcumulador(dataTotales.getAcumulador() < dataTotales.getValorOrdenador() ? dataTotales.getAcumulador() : dataTotales.getValorOrdenador());
			else if (dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_SUM))
				dataTotales.setAcumulador(dataTotales.getAcumulador() + dataTotales.getValorOrdenador());
			...
			if (dato.isPorcentaje())
				dataTotales.setValue(new JFloat(dataTotales.getAcumulador()).setPrec(2).toFormattedString());
			else if (dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_AVR))
				dataTotales.setValue(new JFloat(dataTotales.getAcumulador() / dataTotales.getCantidad()).setPrec(2).toFormattedString());
			else if (value.isFloat() || value.isCurrency())
				dataTotales.setValue(new JFloat(dataTotales.getAcumulador()).setPrec(2).toFormattedString());
			else
				dataTotales.setValue(new JLong((long) dataTotales.getAcumulador()).toFormattedString());
			colMatrixT.setCantidad(colMatrixT.getCantidad() + 1);

			if (dato.isPorcentaje())
				colMatrixT.setValor(colMatrixT.getValor() + dataTotales.getValorOrdenador());
			else if (dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_MAX))
				colMatrixT.setValor(colMatrixT.getValor() > dataTotales.getValorOrdenador() ? colMatrix.getValor() : dataTotales.getValorOrdenador());
			else if (dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_MIN))
				colMatrixT.setValor(colMatrixT.getValor() < dataTotales.getValorOrdenador() ? colMatrix.getValor() : dataTotales.getValorOrdenador());
			else if (dato.getFunctionTotalizadora().equals(BizCampo.FUNTION_SUM))
				colMatrixT.setValor(colMatrixT.getValor() + dataTotales.getValorOrdenador());
...
//			data.setEje(eje);
		...
		boolean hasTotalizador(JColumnaLista infoColumna) throws Exception {
			...
			if (!displayTotales)
				return false;
			if (infoColumna != null && !Number.class.isAssignableFrom(infoColumna.GetClase()))
				return false;
		...
		boolean hasSubTotalizador() throws Exception {
			if (!getWins().matrixDisplaySubTotales())
				return false;
			if (!getWins().matrixDisplayTotales())
				return false;
			return true;
		...
		void fillCeldas(JWin win) throws Exception {
			...
			while (itc.hasMoreElements()) {
				...
				if (campo.equals(""))
					...
					value = win.getRecord().getPropRaw(campo);
				...
				while (it.hasMoreElements()) {
					...
					if (eje.getEje() == null)
						continue;
					if (!eje.getEje().getCalculeDiferencia())
						continue;
					// if (!campo.equals(eje.getEje().getObjCampoDiferencia().getNameField()))
					// continue;
					if (!eje.getEje().getCampoDiferencia().containsElement(campo))
						continue;
...
//			addSubtotal(false);
		...
		/**
		 * Executes the matrix building process. It iterates over the available records,
		 * processes grouping information and fills rows and columns along with totals
		 * and ranking data required by the responsive matrix view.
		 */
