 ...
 * 
 */
public class JPostgreSQL8SystemDBIndex extends JBaseSystemDBIndex {

	/**
	 * @throws Exception
	 * 
	 */
	public JPostgreSQL8SystemDBIndex() throws Exception {
	}
...
//			JExcepcion.SendError("No existe informacion sobre el indice: " + indexName);
			PssLogger.logError("No existe informacion sobre el indice: " + indexName);
			return;
		}
		...
		while (true) {
			...
			if (oReg.next() == false) {
				PssLogger.logError("Error no se puede leer campo con numero de orden: " + String.valueOf(fieldOrdinalNr) + " en el indice " + indexName);
				return;
