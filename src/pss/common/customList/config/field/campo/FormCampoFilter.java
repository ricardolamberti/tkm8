	...
	public void checkControls(String sControlId) throws Exception {
  	...
  	if (s!=null) {
  		if (s.equals(JObject.JDATE) || s.equals(JObject.JDATETIME)) {
  			field="value_date";
   			getControles().findControl("value_edit").setVisible(false);
   			getControles().findControl("value_edit2").setVisible(false);
    		if (f.equals("in")) {
	    	...
	    	}else if (s.equals(JObject.JBOOLEAN) ) {
	   			getControles().findControl("value_edit").setVisible(false);
	   			getControles().findControl("value_edit2").setVisible(false);
	   			getControles().findControl("operador").setVisible(false);
	   			getControles().findControl("funcion").setVisible(false);
	  			field="value_check";
		   	} 
	  	} else {
				field="value_edit";
  	...
  	if (sControlId.equals("id_gallery")) { 
		...
		if (oper!=null) {
			if (oper.getCantValores()>0)
				getControles().findControl(field).setVisible(true);
			else
				getControles().findControl(field).setVisible(false);
			if (oper.getCantValores()>1)
				getControles().findControl(field+"2").setVisible(true);
			else
				getControles().findControl(field+"2").setVisible(false);
		} 
...
//		this.cleanAll();
		...
		try {
			...
			}	if (field.equals("value_lov")) {
   			getControles().findControl("value_edit").setVisible(false);
   			getControles().findControl("value_edit2").setVisible(false);
			}if (field.equals("value_check")) {
   			getControles().findControl("value_edit").setVisible(false);
   			getControles().findControl("value_edit2").setVisible(false);
   			getControles().findControl("operador").setVisible(false);
   			getControles().findControl("funcion").setVisible(false);
