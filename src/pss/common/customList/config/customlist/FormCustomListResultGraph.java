...
public class FormCustomListResultGraph extends JBaseForm {
    ...
    if (getWin().showFilter()) {
			...
			if (getWin().GetcDato().isViewTable())
				if (getWin().GetcDato().sinCategoria())
					row2.AddItemList(100).setMode(JFormLista.MODE_CUSTOMLIST).setShowFooterBar(false).setAllowSortedColumns(false).setForceHidePaginationBar(true);
				else
			...
			JFormPanelResponsive row1 = AddItemColumn(12).setZoomtofit(1500);
			row1.AddItemImage(null, "image").setForceImage(getWin().getMode()==GuiCustomList.EMBEDDED).setHeight(600).setWidth(600);
			if (getWin().GetcDato().isViewTable())
				if (getWin().GetcDato().sinCategoria())
	...
	public void checkControls(String sControlId) throws Exception {
		if (sControlId.equalsIgnoreCase("Aplicar filtros"))
			 getWin().GetcDato().clearCacheImage() ;
