...
//  private boolean bCheckLanguage = false;
	...
	/**
	 * Singleton-pattern method: getInstance(). During the first call of
	 * getInstance(), the languagedocument is parsed. This may result in a rather
	 * long duration of the first call.
	 *
	...
	private String getTranslateMsg(String zKey) {
		try {
			...
			if (result != null) {
				result = result.trim();
				if (JTools.isUpperWord(zKey))
					result = result.toUpperCase();
				return result += sNoTranslate;
			}
			if (addUnknown)
				addToFile(sLang, zKey);
			return result;
		} catch (Exception e) {
	...
	/**
	 * method: setLanguageID(). Initialize the language ID to look for the language
	 * messages.
	 *
	 * @returns void.
	 */
	...
	/**
	 * method: loadFile(). load a file into the message IDs and their translations.
	 *
	 * @returns void.
	 */
...
//    
		...
		if (getLangs().getElement(zLang) == null)
			getLangs().addElement(zLang, JCollectionFactory.<String, String>createOrderedMap(pLangMsgs.size() + 1));
	...
	/*
	 * private boolean isRealNewLine( String zLine ) throws Exception { char
	 * excludes[] =
	 * {'1','2','3','4','5','6','7','8','9','0','[',']','(',')','{','}','<','>','-',
	 * '=','.',':','/','\\'}; int i,j,t=zLine.length(); for (i=0;i<t;i++) for
	 * (j=0;j<excludes.length;j++) if (zLine.charAt(i)==excludes[j]) return false;
	 * return true; }
	 */
...
//      return;
	...
	private synchronized void initLanguage(String zLang) throws Exception {
		...
		if (url == null)
			url = JCompanyBusinessModules.createBusinessDefault().getPathMultilanguage(zLang);
		if (url == null)
			url = JLanguage.class.getResource(zLang);
		if (url == null)
			return;
	...
	public static String translateExtendedText(String zMsg) {
		...
		for (i = 0; i < iLen; i++) {
			...
			if (!((c >= 'A' && c <= 'Z') || (c == '�' || c == '�' || c == '�' || c == '�' || c == '�') || (c == '�' || c == '�' || c == '�' || c == '�' || c == '�') || (c >= 'a' && c <= 'z') || (c == '�') || (c == '?') || (c == '�') || (c == ' ' && sAux.length() > 0))) {
...
//    if ( j > 0 ) {
	...
	public static JRecords<BizVirtual> ObtenerIdiomas() throws Exception {
		...
		oBDs.addItem(JRecord.virtualBD("ar", "Espa�ol (Argentina)", 1));
		oBDs.addItem(JRecord.virtualBD("cl", "Espa�ol (Chile)", 1));
		oBDs.addItem(JRecord.virtualBD("gt", "Espa�ol (Guatemala)", 1));
		oBDs.addItem(JRecord.virtualBD("mx", "Espa�ol (M�xico)", 1));
		oBDs.addItem(JRecord.virtualBD("br", "Portugu�s (Brasil)", 1));
		oBDs.addItem(JRecord.virtualBD("us", "Ingl�s (USA)", 1));
		return oBDs;
	}
