...
public class BizMailSender extends JRecord {
	...
	public JString pMailBcc = new JString();

	public void clearMailBCC() {
		pMailBcc.setValue("");
	}

	public void setMailBCC(String aMailTo) throws Exception {
		if (pMailBcc.isNull() || pMailBcc.getValue().equalsIgnoreCase("")) {
			pMailBcc.setValue(aMailTo);
		} else {
			pMailBcc.setValue(pMailBcc.getValue() + "," + aMailTo);
		}
	}
	...
	public void send(String title, String body, String filename) throws Exception {
		...
		// session.getTransport(oConf.getTransport());
...
//		if (pMailBcc.isNotNull()) {
//			PssLogger.logDebug("mailbcc: " + pMailBcc.getValue());
//
//			final Address[] bcc = InternetAddress.parse(pMailBcc.getValue());
...
//		}
		...
		if (vFiles.isEmpty() == false) {
			Iterator<String> it = vFiles.iterator();
			while (it.hasNext()) {
				String file = it.next();
				MimeBodyPart mbp2 = new MimeBodyPart();
	 ...
	 * "smtp.kar.com"; String from = "FromAddre@kar.com"; String[] to =
	 * {"ABC@kar.com","XYZ@kar.com"}; String filename = "AttachFile.txt"; // Get
	 * system properties Properties props = System.getProperties();
	 ...
	 * 
	 * InternetAddress[] toAddress = new InternetAddress[to.length]; for (int i
	 * = 0; i < to.length; i++) toAddress[i] = new InternetAddress(to[i]);
	 * message.setRecipients(Message.RecipientType.TO, toAddress);
	 * message.setSubject("Hello JavaMail Attachment");
	 * 
	 * BodyPart messageBodyPart = new MimeBodyPart();
	 * messageBodyPart.setText("Here's the file"); Multipart multipart = new
	 * MimeMultipart(); multipart.addBodyPart(messageBodyPart); messageBodyPart
	 * = new MimeBodyPart(); DataSource source = new FileDataSource(filename);
	 * messageBodyPart.setDataHandler(new DataHandler(source));
	 * messageBodyPart.setFileName(filename);
	 * multipart.addBodyPart(messageBodyPart); message.setContent(multipart);
	 * 
	 * try{ Transport.send(message); } catch(SendFailedException sfe) {
	 * message.setRecipients(Message.RecipientType.TO,
	 * sfe.getValidUnsentAddresses()); Transport.send(message);
	 * 
	 * } }
	 * 
	 * }
